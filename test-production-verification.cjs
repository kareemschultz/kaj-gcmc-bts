...
class ProductionVerifier {
  ...
  async verifyLoginPageBranding() {
    await this.page.goto(`${TEST_CONFIG.baseUrl}/login`, { waitUntil: 'networkidle0' });
  ...
  async verifyServerEndpoints() {
    ...
    if (trpcData.result?.data !== 'OK') {
      throw new Error(`tRPC health response invalid: ${JSON.stringify(trpcData)}`);
    }
  }
  ...
  async verifyAuthentication() {
    ...
    await this.page.type('input[type="password"]', TEST_CONFIG.credentials.password);
    ...
    console.log(`🔐 Login attempt completed - error present: ${hasErrorMessage}`);
  }
  ...
  async runAllTests() {
    try {
      ...
      await this.test('Homepage Redirect to Login', () => this.verifyHomepageRedirect());
      await this.test('Login Page Branding & UI', () => this.verifyLoginPageBranding());
      await this.test('Server API Endpoints', () => this.verifyServerEndpoints());
