...
export const GRA_TAX_RATES = {
	CORPORATE_TAX: {
		STANDARD: 0.25, // 25%
		SMALL_BUSINESS: 0.1, // 10% for businesses with turnover < GYD 15M
		MANUFACTURING: 0.2, // 20% for manufacturing
	},
	VAT: {
		STANDARD: 0.14, // 14%
		ZERO_RATED: 0.0, // Exports, essential items
		EXEMPT: 0.0, // Financial services, education, health
	},
	WITHHOLDING_TAX: {
		DIVIDENDS: 0.2, // 20%
		INTEREST: 0.2, // 20%
 ...
 */
export const GRA_FILING_REQUIREMENTS = [
	{
		id: "GRA_CIT_ANNUAL",
		agency: "GRA",
 ...
 */
export function calculateGRADeadlines(business) {
	const deadlines = [];
	const now = new Date();
	...
	if (["CORPORATION", "BRANCH", "SUBSIDIARY"].includes(business.businessType)) {
		...
		deadlines.push({
			...
			daysUntilDue: Math.ceil(
				(citDueDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24),
			),
			penalties: {
	...
	if (business.annualRevenue >= 10000000) {
		const vatDueDate = addDays(
			addMonths(new Date(currentYear, now.getMonth(), 1), 1),
			14,
		);
		deadlines.push({
			agency: "GRA",
			...
			daysUntilDue: Math.ceil(
				(vatDueDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24),
			),
			penalties: {
	...
	if (business.employeeCount > 0) {
		...
		deadlines.push({
			...
			daysUntilDue: Math.ceil(
				(whtDueDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24),
			),
			penalties: {
 ...
 */
export function calculateGRATaxes(input) {
	const {
		grossIncome,
		deductions,
	...
	const witholdingTax =
		estimatedPayments * GRA_TAX_RATES.WITHHOLDING_TAX.SERVICES;
 ...
 */
export function assessGRACompliance(business, _filingHistory = []) {
 ...
 */
export function getGRAForms(businessType) {
	const forms = {
		CORPORATION: [
