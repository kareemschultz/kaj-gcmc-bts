...
exports.SecureSchemas = {
    email: zod_1.z
        .string()
        .email()
        .transform(function (email) { return email.toLowerCase().trim(); }),
    password: zod_1.z
        .string()
    ...
    safeText: zod_1.z
        ...
        .regex(exports.ValidationPatterns.SAFE_TEXT, "Text contains invalid characters")
        .transform(sanitizeText),
    businessName: zod_1.z
        .string()
        .min(1, "Business name required")
        .max(100, "Business name too long")
        .regex(exports.ValidationPatterns.BUSINESS_NAME, "Business name contains invalid characters")
        .transform(sanitizeText),
    fileName: zod_1.z
        .string()
        .refine(function (name) { return sanitizeFileName(name).isValid; }, {
        message: "Invalid file name",
    })
        .transform(function (name) { return sanitizeFileName(name).sanitized; }),
    phoneNumber: zod_1.z
        .string()
        .regex(exports.ValidationPatterns.PHONE, "Invalid phone number format")
        .transform(function (phone) { return phone.replace(/\D/g, ""); }), // Remove non-digits
    postalCode: zod_1.z
        .string()
        .regex(exports.ValidationPatterns.POSTAL_CODE, "Invalid postal code format")
        .transform(function (code) { return code.toUpperCase().trim(); }),
    htmlContent: zod_1.z
        .string()
    ...
    sqlIdentifier: zod_1.z
        .string()
        .regex(exports.ValidationPatterns.SQL_IDENTIFIER, "Invalid identifier format"),
    tenantCode: zod_1.z
        .string()
        ...
        .transform(function (code) { return code.toLowerCase().trim(); }),
    ipAddress: zod_1.z.string().refine(validator_1.default.isIP, "Invalid IP address"),
    url: zod_1.z
        .string()
