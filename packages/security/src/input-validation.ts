 ...
 */
export function sanitizeFileName(fileName: string): { isValid: boolean; sanitized?: string; errors?: string[] } {
	const errors: string[] = [];
...
export const SecureSchemas = {
	...
	businessName: z.string()
		...
		.regex(ValidationPatterns.BUSINESS_NAME, 'Business name contains invalid characters')
		.transform(sanitizeText),
	...
	ipAddress: z.string()
		.refine(validator.isIP, 'Invalid IP address'),
 ...
 */
export function validateJsonInput(input: string): { isValid: boolean; parsed?: any; error?: string } {
	try {
		// Check for suspicious patterns
		if (input.includes('__proto__') || input.includes('constructor') || input.includes('prototype')) {
			return { isValid: false, error: 'JSON contains potentially dangerous properties' };
		}
		...
		if (typeof parsed === 'object' && parsed !== null) {
			if ('__proto__' in parsed || 'constructor' in parsed || 'prototype' in parsed) {
				return { isValid: false, error: 'JSON contains prototype pollution attempt' };
			}
		}
 ...
 */
export function validateCommandArgument(arg: string): { isValid: boolean; error?: string } {
