...
export class MultiLevelCacheService implements CacheService {
  ...
  // Metrics
  private cacheHitsCounter = metrics.createCounter('cache_hits_total', 'Total cache hits');
  private cacheMissesCounter = metrics.createCounter('cache_misses_total', 'Total cache misses');
  private cacheSetsCounter = metrics.createCounter('cache_sets_total', 'Total cache sets');
  private cacheDeletesCounter = metrics.createCounter('cache_deletes_total', 'Total cache deletes');
  private cacheOperationTimer = metrics.createTimer('cache_operation', 'Cache operation duration');
  ...
  async get<T>(key: string, options: CacheOptions = {}): Promise<T | null> {
    ...
    return this.cacheOperationTimer.timeAsync(async () => {
      // Try L1 cache first
  ...
  async delete(key: string, options: CacheOptions = {}): Promise<void> {
    ...
    return this.cacheOperationTimer.timeAsync(async () => {
      ...
      try {
        ...
        logger.error('Redis cache delete error', error, { key: namespacedKey });
      }
  ...
  async invalidateByTags(tags: string[], options: CacheOptions = {}): Promise<void> {
    const namespace = options.namespace || 'default';
    ...
    return this.cacheOperationTimer.timeAsync(async () => {
      ...
      for (const tag of tags) {
        ...
        try {
          ...
          logger.error('Redis tag invalidation error', error, { tag: tagKey });
        }
      }
  ...
  private cleanupL1Cache(): void {
    ...
    if (cleanedCount > 0) {
      logger.debug('L1 cache cleanup', { cleanedCount, remainingSize: this.l1Cache.size });
    }
  }
}
