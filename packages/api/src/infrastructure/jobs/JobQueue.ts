...
export class JobQueue<T extends JobData = JobData> {
  ...
  // Metrics
  private jobCounter = metrics.createCounter('jobs_total', 'Total jobs processed');
  private jobDurationHistogram = metrics.createHistogram('job_duration_seconds', 'Job processing duration');
  private jobFailureCounter = metrics.createCounter('job_failures_total', 'Total job failures');
  private activeJobsGauge = metrics.createGauge('jobs_active', 'Currently active jobs');
  ...
  async addJob(
    ...
    this.jobCounter.increment({ queue: this.queueName, type: jobType, status: 'added' });
  ...
  async addBulkJobs(
    ...
    this.jobCounter.add(jobs.length, { queue: this.queueName, status: 'bulk_added' });
