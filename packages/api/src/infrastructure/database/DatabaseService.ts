...
export class DatabaseService {
	...
	// Metrics
	private queryCounter = metrics.createCounter(
		"database_queries_total",
		"Total database queries executed",
	);
	private queryDurationHistogram = metrics.createHistogram(
		"database_query_duration_seconds",
		"Database query duration",
	);
	private connectionGauge = metrics.createGauge(
	...
	async executeQuery<T>(
		...
		try {
			...
			this.stats.avgQueryDuration =
				(this.stats.avgQueryDuration * (this.stats.queriesExecuted - 1) +
					duration) /
				this.stats.queriesExecuted;
	...
	async validateSchema(): Promise<boolean> {
		try {
			...
			await this.prisma
				.$queryRaw`SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' LIMIT 1`;
			return true;
		} catch (error) {
