...
export class ServiceRegistry {
	...
	private async registerInfrastructure(): Promise<void> {
		...
		this.container.instance(
			ServiceTokens.EVENT_DISPATCHER,
	...
	private registerApplicationServices(): void {
		...
		this.container.singleton(ServiceTokens.CLIENT_SERVICE, () => {
			const clientRepository = this.container.resolve(
				ServiceTokens.CLIENT_REPOSITORY,
			);
	...
	private registerEventHandlers(): void {
		...
		const clientCreatedHandler = new ClientCreatedEventHandler(
			cacheService,
			auditService,
		);
		const clientUpdatedHandler = new ClientUpdatedEventHandler(
			cacheService,
			auditService,
		);
		const clientRiskChangedHandler = new ClientRiskLevelChangedEventHandler(
			cacheService,
		);
	...
	private async initializeServices(): Promise<void> {
		...
		const cacheService = this.container.resolve<MultiLevelCacheService>(
...
export function getServiceRegistry(): ServiceRegistry {
	if (!serviceRegistry) {
		throw new Error(
			"Service registry not initialized. Call initializeServices() first.",
		);
	}
