...
export class AgencyValidationService {
  ...
  private async validateAgainstRule(
    rule: any,
    documentData: DocumentData
  ): Promise<ValidationError | ValidationWarning | ValidationSuggestion | null> {
  ...
  private validateSize(
    ...
    if (operator === "range") {
      if ((min !== undefined && size < min) || (max !== undefined && size > max)) {
        return {
          ruleId: rule.id,
          field,
          message: errorMessage || `${field} size must be between ${min} and ${max}`,
          severity: "error",
  ...
  private async applyAuthoritySpecificRules(
    ...
    switch (authority) {
      case "GRA":
        await this.applyGRASpecificRules(documentType, documentData, validationResult);
        break;

      case "NIS":
        await this.applyNISSpecificRules(documentType, documentData, validationResult);
        break;

      case "DCRA":
        await this.applyDCRASpecificRules(documentType, documentData, validationResult);
        break;

      case "Immigration":
        await this.applyImmigrationSpecificRules(documentType, documentData, validationResult);
        break;
  ...
  private async applyGRASpecificRules(
    ...
    if (documentType === "VAT_RETURN") {
      ...
      const totalSales = this.getFieldValue(documentData, "totalSales") as number;
    ...
    if (documentType === "INCOME_TAX_RETURN") {
      ...
      const grossIncome = this.getFieldValue(documentData, "grossIncome") as number;
      const taxPayable = this.getFieldValue(documentData, "taxPayable") as number;
  ...
  private async applyNISSpecificRules(
    ...
    if (documentType === "EMPLOYEE_CONTRIBUTION") {
      ...
      const contribution = this.getFieldValue(documentData, "employeeContribution") as number;
  ...
  private async applyDCRASpecificRules(
    ...
    if (documentType === "COMPANY_REGISTRATION") {
      const companyName = this.getFieldValue(documentData, "companyName") as string;
  ...
  private async applyImmigrationSpecificRules(
    ...
    if (documentType === "WORK_PERMIT_APPLICATION") {
      const passportExpiry = this.getFieldValue(documentData, "passportExpiryDate");
      ...
      if (passportExpiry) {
        ...
        if (expiryDate < sixMonthsFromNow) {
          validationResult.warnings.push({
            ...
            message: "Passport expires within 6 months. Consider renewal before application.",
            severity: "warning",
   ...
   */
  private evaluateCondition(fieldValue: unknown, condition: WorkflowCondition): boolean {
 ...
 */
export function createAgencyValidationService(prisma: any): AgencyValidationService {
