...
export class SmartDocumentService {
	...
	async getDocumentRecommendations(
		...
		try {
			...
			const recommendations = {
				...
				missingDocuments: await this.identifyMissingDocuments(
					complianceStatus,
					requirements,
				),
				complianceIssues: await this.identifyComplianceIssues(complianceStatus),
				deadlineAlerts: await this.getDeadlineAlerts(
					tenantId,
					clientId,
					authority,
				),
		...
		const patterns = {
			...
			NIS: {
				EMPLOYEE_CONTRIBUTION:
					/nis\s*contribution|national\s*insurance|employee\s*contribution/i,
				EMPLOYER_RETURN: /employer\s*return|nis\s*return|social\s*security/i,
			},
			DCRA: {
				COMPANY_REGISTRATION:
					/company\s*registration|incorporation|memorandum|articles\s*of\s*association/i,
				ANNUAL_RETURN: /annual\s*return|company\s*return|form\s*18/i,
				DIRECTOR_CHANGE: /director\s*change|resignation|appointment/i,
			},
			Immigration: {
				WORK_PERMIT:
					/work\s*permit|employment\s*authorization|labour\s*permit/i,
	 ...
	 */
	private async extractTablesFromText(textContent: string): Promise<
		Array<{
			name: string;
			headers: string[];
			rows: string[][];
	 ...
	 */
	private async detectSignatures(textContent: string): Promise<
		Array<{
			location: string;
			confidence: number;
		}>
	 ...
	 */
	private async detectStamps(textContent: string): Promise<
		Array<{
			text: string;
			location: string;
			confidence: number;
	...
	private getSuggestedActions(
		...
		return (
			actionMap[authority]?.[documentType] || [
				"Review document",
