...
export class ComplianceAlertService {
  ...
  async processDeadlineAlerts(tenantId: number): Promise<ComplianceAlert[]> {
    ...
    for (const deadline of deadlines) {
      try {
        ...
        const severity = this.calculateAlertSeverity(daysUntilDue, deadline.priority);
  ...
  async checkPenaltyAccumulation(tenantId: number): Promise<ComplianceAlert[]> {
    ...
    for (const client of clients) {
      try {
        ...
        console.error(`Error checking penalties for client ${client.id}:`, error);
      }
    }
  ...
  async analyzeComplianceScores(tenantId: number): Promise<{
    ...
    const complianceStatuses = await this.prisma.agencyComplianceStatus.findMany({
      where: { tenantId },
      include: {
        client: true,
        agencyInfo: true,
    ...
    for (const status of complianceStatuses) {
      try {
        ...
        console.error(`Error analyzing compliance for client ${status.clientId}:`, error);
      }
    }
  ...
  async generateSystemAlerts(tenantId: number): Promise<ComplianceAlert[]> {
    ...
    try {
      ...
      if (systemMetrics.overduePercentage > 0.2) { // 20%
    ...
    for (const status of complianceStatuses) {
      ...
      for (const reqStatus of status.requirementStatuses) {
        if (reqStatus.status === "non_compliant") {
          ...
          insight.actionItems.push(`Submit ${reqStatus.requirement.documentType}`);
      ...
      if (insight.issues.length > 0) {
        ...
        if (insight.riskLevel === "red") {
          insight.recommendations.push("URGENT: Immediate action required to avoid penalties");
        }
      }
  ...
  private async getDeadlinesNeedingAlerts(tenantId: number) {
    ...
    futureDate.setDate(futureDate.getDate() + Math.max(...this.config.deadlineWarningDays));
  ...
  private generateDeadlineAlertTitle(alertType: string, daysUntilDue: number): string {
    switch (alertType) {
  ...
  private generateDeadlineAlertMessage(deadline: any, daysUntilDue: number): string {
  ...
  private async sendAlertNotifications(alert: ComplianceAlert, deadline: any): Promise<void> {
  ...
  private async updateOverduePenalty(overdueItemId: number, penalty: number): Promise<void> {
  ...
  private shouldEscalateOverdueItem(item: any, currentPenalty: number): boolean {
    return currentPenalty >= this.config.penaltyThresholds[1] || item.daysOverdue >= 30;
  }
  ...
  private async createOverdueEscalationAlert(
    ...
    const alert = await this.prisma.complianceAlert.create({
      data: {
        ...
        severity: penalty >= this.config.penaltyThresholds[2] ? "critical" : "high",
  ...
  private async sendEscalationNotifications(alert: ComplianceAlert, item: any): Promise<void> {
  ...
  private async createPenaltyAlert(
    ...
    const alert = await this.prisma.complianceAlert.create({
      data: {
        ...
        type: "penalty_accruing",
        severity: totalPenalties >= this.config.penaltyThresholds[3] ? "critical" : "high",
  ...
  private isComplianceScoreDeclining(currentScore: number, previousScores: number[]): boolean {
  ...
  private async createComplianceDeclineAlert(tenantId: number, status: any): Promise<void> {
  ...
  private async createCriticalComplianceAlert(tenantId: number, status: any): Promise<void> {
    await this.prisma.complianceAlert.create({
      data: {
        tenantId,
        clientId: status.clientId,
        ...
        message: "Client has critical compliance issues requiring immediate attention",
  ...
  private async calculateSystemMetrics(tenantId: number) {
    ...
    const averageComplianceScore = complianceStatuses.length > 0
      ? complianceStatuses.reduce((sum, cs) => sum + cs.score, 0) / complianceStatuses.length
      : 0;

    return {
      overduePercentage: totalDeadlines > 0 ? overdueDeadlines / totalDeadlines : 0,
