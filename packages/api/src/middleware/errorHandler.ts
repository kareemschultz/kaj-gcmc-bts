...
export class ErrorHandler {
	private static errorCounter = metrics.createCounter(
		"api_errors_total",
		"Total API errors",
	);
	private static errorDurationHistogram = metrics.createHistogram(
	...
	static handleError(error: unknown, context: ErrorContext = {}): TRPCError {
		...
		try {
			...
			const correlationId = requestId || ErrorHandler.generateCorrelationId();

			let trpcError: TRPCError;

			if (error instanceof DomainError) {
				trpcError = ErrorHandler.handleDomainError(error, correlationId);
			} else if (error instanceof TRPCError) {
				trpcError = ErrorHandler.handleTRPCError(error, correlationId);
			} else if (error instanceof Error) {
				trpcError = ErrorHandler.handleGenericError(error, correlationId);
			} else {
				trpcError = ErrorHandler.handleUnknownError(error, correlationId);
			}
			...
			if (ErrorHandler.isServerError(trpcError)) {
				logger.error(
			...
			ErrorHandler.errorCounter.increment({
				code: trpcError.code,
			...
			ErrorHandler.errorDurationHistogram.observe(duration / 1000);
		}
	}

	private static handleDomainError(
		error: DomainError,
		correlationId: string,
	): TRPCError {
		const code = ErrorHandler.mapDomainErrorToTRPCCode(error);
		...
		(trpcError as any).data = {
			...
			rule:
				error instanceof BusinessRuleViolationError ? error.rule : undefined,
			correlationId,
		};
	...
	private static handleGenericError(
		...
		if (error.name === "PrismaClientKnownRequestError") {
			return ErrorHandler.handlePrismaError(error, correlationId);
		}

		if (error.name === "ZodError") {
			return ErrorHandler.handleZodError(error, correlationId);
		}
		...
		return new TRPCError({
			...
			...(process.env.NODE_ENV !== "production" && {
				data: { originalMessage: error.message, correlationId },
			}),
		});
	}
	...
	private static mapDomainErrorToTRPCCode(
		error: DomainError,
	): TRPCError["code"] {
...
export const createCircuitBreaker =
	ErrorHandler.createCircuitBreaker.bind(ErrorHandler);
