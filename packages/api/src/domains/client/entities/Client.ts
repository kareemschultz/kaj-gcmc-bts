...
export class Client {
	...
	get tin(): string | undefined {
		return this._tin;
	}
	get nisNumber(): string | undefined {
		return this._nisNumber;
	}
	...
	updateInfo(
		data: Partial<
			Omit<ClientProps, "id" | "tenantId" | "createdAt" | "updatedAt">
		>,
		updatedBy: User,
	): void {
		...
		this._domainEvents.push(
			new DomainEvent("client.updated", {
				clientId: this._id,
	...
	assessRiskLevel(): RiskLevel {
		...
		if (newRiskLevel !== this._riskLevel) {
			...
			this._domainEvents.push(
				new DomainEvent("client.risk_level_changed", {
					clientId: this._id,
					tenantId: this._tenantId,
					oldRiskLevel,
					newRiskLevel,
	...
	private validateClientData(props: ClientProps): void {
		...
		if (props.tin && props.tin.length > 50) {
			throw new DomainError(
				"TIN cannot exceed 50 characters",
				"CLIENT_TIN_TOO_LONG",
			);
		}
		...
		if (
			props.riskLevel &&
			!Object.values(RiskLevel).includes(props.riskLevel)
		) {
	...
	static create(
		...
		client._domainEvents.push(
			new DomainEvent("client.created", {
