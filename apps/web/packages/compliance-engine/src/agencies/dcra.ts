...
import { addYears, addMonths, isBefore, isAfter } from 'date-fns';
import type { 
 ...
 */
export function calculateDCRADeadlines(business: GuyanaBusinessProfile): FilingDeadline[] {
  ...
  if (requirement) {
    const annualReturnDue = addMonths(business.registrationDate, requirement.dueMonths);
...
export function assessDCRACompliance(
  ...
  if (overdueFilings.length > 0) {
    ...
    if (overdueDays > 365) {
      score = 0;
      level = 'CRITICAL';
      notes.push('Annual return is more than 1 year overdue - business may be struck off');
  ...
  const upcomingDeadlines = deadlines.filter(d => !d.isOverdue && d.daysUntilDue <= 30);
  if (upcomingDeadlines.length > 0) {
    notes.push(`Annual return due in ${upcomingDeadlines[0].daysUntilDue} days`);
  }
 ...
 */
export function getDCRAComplianceChecklist(businessType: BusinessType): string[] {
  const commonItems = [
 ...
 */
export function validateBusinessName(proposedName: string, businessType: BusinessType): {
  isValid: boolean;
  issues: string[];
  suggestions: string[];
} {
  ...
  // Prohibited words
  const prohibitedWords = ['bank', 'insurance', 'trust', 'royal', 'government', 'ministry'];
  const nameWords = proposedName.toLowerCase().split(' ');
  
  prohibitedWords.forEach(word => {
    if (nameWords.includes(word)) {
      issues.push(`Name cannot contain the word "${word}" without special approval`);
    }
  });
  ...
  if (requiredSuffixes[businessType]) {
    ...
    if (!hasRequiredSuffix) {
      issues.push(`${businessType} must include one of: ${requiredSuffixes[businessType].join(', ')}`);
      suggestions.push(`Consider: "${proposedName} Inc." or "${proposedName} Limited"`);
    }
  }
