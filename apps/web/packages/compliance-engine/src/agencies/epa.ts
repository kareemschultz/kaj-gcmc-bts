...
const HIGH_IMPACT_SECTORS = ['mining', 'forestry', 'manufacturing', 'petroleum', 'agriculture'];

export function assessEPACompliance(business: GuyanaBusinessProfile): ComplianceResult {
  let score = 100;
  ...
  if (HIGH_IMPACT_SECTORS.includes(business.sector.toLowerCase())) {
    score = 50;
    level = 'MAJOR_ISSUES';
    notes.push('Business sector requires environmental impact assessment and permit');
  }
