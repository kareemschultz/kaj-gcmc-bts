...
export function calculateNISContributions(
...
): { employeeContribution: number; employerContribution: number; total: number } {
  ...
  const employeeContribution = insurableEarnings * NIS_RATES.EMPLOYEE_CONTRIBUTION;
  const employerContribution = insurableEarnings * NIS_RATES.EMPLOYER_CONTRIBUTION;
...
export function calculateSelfEmployedNIS(
  annualIncome: number
): { contribution: number; quarterly: number; monthly: number } {
  
  const adjustedIncome = Math.max(annualIncome, NIS_RATES.MINIMUM_ANNUAL_SELF_EMPLOYED);
 ...
 */
export function calculateNISDeadlines(business: GuyanaBusinessProfile): FilingDeadline[] {
  const deadlines: FilingDeadline[] = [];
  const now = new Date();
  
  if (business.employeeCount > 0) {
    // Monthly remittance due 15th of following month
    const monthlyDue = addDays(addMonths(new Date(now.getFullYear(), now.getMonth(), 1), 1), 14);
...
export function assessNISCompliance(
  ...
  if (business.businessType === 'SOLE_PROPRIETORSHIP' && business.annualRevenue >= NIS_RATES.MINIMUM_ANNUAL_SELF_EMPLOYED) {
...
export function calculateAnnualNISBudget(
  ...
  if (business.employeeCount > 0) {
    ...
    const { employeeContribution, employerContribution } = calculateNISContributions(avgWeeklyPerEmployee);
  ...
  return {
    ...
    total: employeeContributions + employerContributions + selfEmployedContributions,
