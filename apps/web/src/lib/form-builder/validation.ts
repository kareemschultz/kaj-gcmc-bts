...
export const AUTHORITY_VALIDATION_RULES: Record<Authority, Record<string, ValidationRule[]>> = {
  GRA: {
    tin: [
      {
        id: "gra_tin_format",
        ...
        guyaneseRule: {
          ...
          parameters: {
            brackets: [
              ...
              { min: 1560001, max: Infinity, rate: 0.40 }
            ]
          }
        }
      }
    ...
    business_registration: [
      {
        ...
        type: "guyaneseSpecific",
        message: "Business registration must start with 'RC' followed by 6 digits",
        severity: "error",
        guyaneseRule: {
          authority: "DCRA",
...
export const createFieldValidationSchema = (field: FormField): z.ZodType<unknown> => {
  ...
  switch (field.type) {
    ...
    case "url":
      ...
      if (textField.minLength) {
        schema = (schema as z.ZodString).min(textField.minLength, `Minimum length is ${textField.minLength}`);
      }
      if (textField.maxLength) {
        schema = (schema as z.ZodString).max(textField.maxLength, `Maximum length is ${textField.maxLength}`);
      }
      if (textField.pattern) {
        schema = (schema as z.ZodString).regex(new RegExp(textField.pattern), "Invalid format");
      }
      break;
    ...
    case "percentage":
      ...
      if (typeof numberField.min === "number") {
        schema = (schema as z.ZodNumber).min(numberField.min, `Minimum value is ${numberField.min}`);
      }
      if (typeof numberField.max === "number") {
        schema = (schema as z.ZodNumber).max(numberField.max, `Maximum value is ${numberField.max}`);
      }
      break;
    ...
    case "datetime":
    case "time":
      schema = z.string().refine((date) => !isNaN(Date.parse(date)), "Invalid date format");
      break;

    case "select":
    case "radio":
      const selectField = field as any;
      const allowedValues = selectField.options?.map((opt: any) => opt.value) || [];
      schema = z.enum(allowedValues.length > 0 ? [allowedValues[0], ...allowedValues.slice(1)] : ["default"]);
      break;

    case "multiselect":
    case "checkboxGroup":
      const multiSelectField = field as any;
      const multiAllowedValues = multiSelectField.options?.map((opt: any) => opt.value) || [];
      schema = z.array(z.enum(multiAllowedValues.length > 0 ? [multiAllowedValues[0], ...multiAllowedValues.slice(1)] : ["default"]));
      break;
    ...
    case "business_reg":
      schema = z.string().regex(GUYANESE_PATTERNS.DCRA_REG, "Invalid business registration format");
      break;

    case "nis_number":
      schema = z.string().regex(GUYANESE_PATTERNS.NIS_NUMBER, "Invalid NIS number format");
      break;

    case "passport":
      schema = z.string().regex(GUYANESE_PATTERNS.PASSPORT, "Invalid passport number format");
      break;

    case "phone":
      schema = z.string().regex(GUYANESE_PATTERNS.PHONE_GY, "Invalid phone number format");
      break;
  }
  ...
  if (field.required) {
    if (schema instanceof z.ZodString) {
      ...
      schema = schema.refine((val) => val !== null && val !== undefined, "This field is required");
    }
  } else {
...
export class FormValidationEngine {
   ...
   */
  validateFieldRealTime(fieldId: string, value: unknown, allValues: Record<string, unknown> = {}): ValidationError[] {
    const errors = this.validateField(fieldId, value);

    // Check conditional validations that might affect this field
    const conditionalErrors = this.validateConditionalRules(fieldId, value, allValues);
    errors.push(...conditionalErrors);

    return errors;
  }

  private applyValidationRule(rule: ValidationRule, fieldId: string, value: unknown): ValidationError | null {
    switch (rule.type) {
      case "required":
  ...
  private validateGuyaneseSpecificRule(rule: ValidationRule, fieldId: string, value: unknown): ValidationError | null {
  ...
  private validateBusinessRule(rule: ValidationRule, fieldId: string, value: unknown): ValidationError | null {
    if (!rule.guyaneseRule) return null;
  ...
  private validateCrossFieldRules(formData: Partial<FormData>): ValidationError[] {
    const errors: ValidationError[] = [];
    ...
    this.formConfig.fields.forEach(field => {
      field.validationRules?.forEach(rule => {
        if (rule.type === "crossField" && rule.field) {
          ...
          const error = this.validateCrossFieldRule(rule, field.id, currentValue, relatedValue);
          if (error) {
            errors.push(error);
          }
        }
  ...
  private validateCrossFieldRule(rule: ValidationRule, fieldId: string, currentValue: unknown, relatedValue: unknown): ValidationError | null {
    // Implement specific cross-field validation logic
    // This would depend on the specific rule and business requirements
    return null;
  }

  private validateBusinessRules(formData: Partial<FormData>): ValidationError[] {
    const errors: ValidationError[] = [];
  ...
  private validateConditionalRules(fieldId: string, value: unknown, allValues: Record<string, unknown>): ValidationError[] {
    const errors: ValidationError[] = [];
  ...
  getValidationSummary(formData: Partial<FormData>): {
    ...
    fieldSummary: Record<string, { isValid: boolean; errors: ValidationError[] }>;
  } {
    ...
    const fieldSummary: Record<string, { isValid: boolean; errors: ValidationError[] }> = {};
    this.formConfig.fields.forEach(field => {
 ...
 */
export const createValidationEngine = (formConfig: FormConfiguration): FormValidationEngine => {
