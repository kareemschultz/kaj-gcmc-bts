...
export const TAX_CONSTANTS = {
  ...
  COMMON: {
    ...
    EXCHANGE_RATES: {
      USD_TO_GYD: 209.00, // Approximate rate
      EUR_TO_GYD: 225.00,
      GBP_TO_GYD: 260.00,
      CAD_TO_GYD: 155.00
    }
...
export class CalculationEngine {
  ...
  private calculateSubtraction(config: CalculationConfig): number {
    const { minuend, subtrahends } = config.parameters as { minuend: string; subtrahends: string[] };
  ...
  private calculateDivision(config: CalculationConfig): number {
    const { dividend, divisor } = config.parameters as { dividend: string; divisor: string };
    ...
    return (typeof dividendValue === "number" ? dividendValue : 0) / divisorValue;
  }

  private calculatePercentage(config: CalculationConfig): number {
    const { value, percentage } = config.parameters as { value: string; percentage: number };
  ...
  private calculateCompound(config: CalculationConfig): number {
    const { principal, rate, time, frequency = 1 } = config.parameters as {
      principal: string;
      rate: number;
      time: number;
      frequency?: number;
    ...
    return principalValue * Math.pow(1 + (rate / frequency), frequency * time);
  }
  ...
  private calculateVAT(config: CalculationConfig): number {
    ...
    const { baseAmount, vatType, isInclusive = false } = config.parameters as {
      baseAmount: string;
      vatType?: string;
      isInclusive?: boolean;
    };
  ...
  private calculateNIS(config: CalculationConfig): number {
    ...
    const { salary, contributionType, period = "monthly" } = config.parameters as {
      salary: string;
    ...
    // Apply wage limits
    const contributableSalary = Math.min(Math.max(salaryValue, minWage), maxWage);
  ...
  private calculateWithholdingTax(config: CalculationConfig): number {
    const guyaneseCalc = config.guyaneseCalculation;
    if (!guyaneseCalc || guyaneseCalc.authority !== "GRA") {
      throw new Error("Withholding tax calculation requires GRA authority configuration");
    }

    const { amount, incomeType } = config.parameters as {
      amount: string;
      incomeType: "dividends" | "interest" | "royalties" | "management_fees" | "technical_services";
    };
  ...
  private calculatePenalty(config: CalculationConfig): number {
    ...
    switch (penaltyType) {
      case "late_filing":
        ...
        return baseValue * TAX_CONSTANTS.COMMON.PENALTY_RATES.LATE_FILING * monthsLate;

      case "late_payment":
        if (!daysLate) return 0;
        const paymentMonthsLate = Math.ceil(daysLate / 30);
        return baseValue * TAX_CONSTANTS.COMMON.PENALTY_RATES.LATE_PAYMENT * paymentMonthsLate;
  ...
  private calculateFees(config: CalculationConfig): number {
    ...
    const { feeType, entityType } = config.parameters as {
      ...
      entityType?: "company" | "partnership" | "sole_proprietorship" | "foreign_company";
    };
  ...
  private calculateCurrencyConversion(config: CalculationConfig): number {
    ...
    switch (fromCurrency) {
      case "GYD":
        ...
        break;
      case "USD":
        gydAmount = amountValue * TAX_CONSTANTS.COMMON.EXCHANGE_RATES.USD_TO_GYD;
        break;
      case "EUR":
        gydAmount = amountValue * TAX_CONSTANTS.COMMON.EXCHANGE_RATES.EUR_TO_GYD;
        break;
      case "GBP":
        gydAmount = amountValue * TAX_CONSTANTS.COMMON.EXCHANGE_RATES.GBP_TO_GYD;
        break;
      case "CAD":
        gydAmount = amountValue * TAX_CONSTANTS.COMMON.EXCHANGE_RATES.CAD_TO_GYD;
        break;
      default:
        return 0;
    }
  ...
  private createFormulaContext(): Record<string, unknown> {
    ...
    context.avg = (...values: number[]) => values.reduce((a, b) => a + b, 0) / values.length;
    context.min = Math.min;
    context.max = Math.max;
    context.round = (value: number, decimals = 2) => Math.round(value * Math.pow(10, decimals)) / Math.pow(10, decimals);
  ...
  static calculateIncomeTax(annualIncome: number): {
    ...
    marginalRate: number;
    breakdown: Array<{ bracket: string; taxableAmount: number; rate: number; tax: number }>;
  } {
    ...
    const marginalRate = brackets.find(b => annualIncome > b.min && annualIncome <= b.max)?.rate || 0;
 ...
 */
export const createCalculationEngine = (formData: FormData, fields: FormField[]): CalculationEngine => {
