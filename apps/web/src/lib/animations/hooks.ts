...
export function usePerformanceAnimation() {
	...
	const [performanceMetrics, setPerformanceMetrics] =
		useState<PerformanceMetrics>(() => performanceMonitor.getMetrics());
	...
	const shouldReduceAnimations =
		performanceMetrics.frameRate < 45 || performanceMetrics.animationCount > 15;

	return {
		performanceMetrics,
		shouldAnimate: config.enabled && performanceMetrics.frameRate > 30,
		shouldUseGPU:
			config.enableGPUAcceleration && performanceMetrics.frameRate > 45,
		shouldReduceComplexity: shouldReduceAnimations,
		adaptAnimation: useCallback(
				...
				if (shouldReduceAnimations) {
					return {
						...
						transition: {
							...
							duration: Math.min(
								baseAnimation.transition?.duration || 0.3,
								0.2,
							),
						},
					};
...
export function useDataVisualizationAnimation(
	data: any[],
	...
	const getChartAnimation = useCallback(
		(index = 0) => {
			if (!config.enabled) return {};
