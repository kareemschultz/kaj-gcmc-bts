...
export class WizardStorageService {
  ...
  static saveToLocalStorage(
    ...
    try {
      const session: WizardSession = {
        ...
        createdAt: new Date(localStorage.getItem(`${WIZARD_STORAGE_PREFIX}${sessionId}-created`) || Date.now()),
        updatedAt: new Date(),
      ...
      // Update index
      this.updateSessionIndex(sessionId, wizardType);
    } catch (error) {
  ...
  static loadFromLocalStorage(sessionId: string): WizardSession | null {
    try {
      const stored = localStorage.getItem(`${WIZARD_STORAGE_PREFIX}${sessionId}`);
      ...
      console.error("Failed to load wizard progress from local storage:", error);
      return null;
    }
  }
  ...
  static removeFromLocalStorage(sessionId: string): void {
    try {
      ...
      console.error("Failed to remove wizard progress from local storage:", error);
    }
  }
  ...
  static getSessionsByType(wizardType: string): WizardSession[] {
    return this.getAllSessions().filter(session => session.wizardType === wizardType);
  }
    ...
    try {
      ...
      for (const session of sessions) {
        if (now > session.expiresAt) {
          this.removeFromLocalStorage(session.sessionId);
        }
      }
    } catch (error) {
  ...
  private static updateSessionIndex(sessionId: string, wizardType: string): void {
    try {
      let index = this.getSessionIndex();
...
export class WizardServerStorageService {
  ...
  static async loadFromServer(sessionId: string): Promise<WizardSession | null> {
    try {
      const response = await fetch(`${this.baseUrl}/${sessionId}`);
  ...
  static async getAllSessions(): Promise<WizardSession[]> {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to get wizard sessions: ${response.statusText}`);
      }
...
export class WizardUnifiedStorageService {
  ...
  static async save(
    ...
    try {
      await WizardServerStorageService.saveToServer(sessionId, wizardType, state);
    } catch (error) {
  ...
  static async load(sessionId: string): Promise<WizardSession | null> {
    try {
      // Try server first
      const serverSession = await WizardServerStorageService.loadFromServer(sessionId);
      if (serverSession) {
