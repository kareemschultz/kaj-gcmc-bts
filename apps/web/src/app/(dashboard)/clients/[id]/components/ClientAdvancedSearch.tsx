...
export function ClientAdvancedSearch({
  ...
  const handleAddFilter = (filter: ClientSearchFilter, value: unknown) => {
    const existingFilterIndex = activeFilters.findIndex((f) => f.id === filter.id);
  ...
  const renderFilterInput = (filter: ClientSearchFilter) => {
    ...
    switch (filter.type) {
      ...
      case "number_range":
        const rangeValue = (existingFilter?.value as { min?: number; max?: number }) || {};
        return (
      ...
      case "date_range":
        const dateValue = (existingFilter?.value as { start?: string; end?: string }) || {};
        return (
          <div className="grid grid-cols-2 gap-2">
            <Input
              type="date"
