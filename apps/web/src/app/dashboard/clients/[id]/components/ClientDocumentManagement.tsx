...
import type { ClientDocument, ClientDocumentCategory } from "@gcmc-kaj/types";
import {
	Calendar,
	Download,
	Eye,
...
export function ClientDocumentManagement({
	...
	const [selectedCategory, setSelectedCategory] = useState<
		ClientDocumentCategory | "all"
	>("all");
	const [viewMode, setViewMode] = useState<ViewMode>("list");
	const [sortBy, setSortBy] = useState<"name" | "date" | "size" | "category">(
		"date",
	);
	...
	const getDocumentIcon = (mimeType: string) => {
		const Icon =
			DOCUMENT_ICONS[mimeType as keyof typeof DOCUMENT_ICONS] || File;
		return Icon;
	};
			...
			<div className={cn("space-y-6", className)}>
						...
						<CardContent className="p-4">
							<div className="flex items-center justify-between">
								<div>
									...
									<p className="text-muted-foreground text-sm">Confidential</p>
								</div>
				...
				<Card>
					<CardHeader>
						...
						<CardDescription>
							Manage all client documents with advanced organization and search
							capabilities
						</CardDescription>
					</CardHeader>
					<CardContent>
													...
													<DropdownMenu>
														...
														<DropdownMenuContent align="end">
															...
															{onUpdateTags && (
																<DropdownMenuItem
																	onClick={() =>
																		onUpdateTags(document.id, document.tags)
																	}
																>
