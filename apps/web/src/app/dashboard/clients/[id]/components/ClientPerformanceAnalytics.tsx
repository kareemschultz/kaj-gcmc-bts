...
export function ClientPerformanceAnalytics({
	...
	const kpiMetrics: KPIMetric[] = useMemo(() => {
		return [
			...
			},
			{
				...
				status:
					analytics.averageResponseTime <= 2
						? "excellent"
						: analytics.averageResponseTime <= 4
							? "good"
							: "warning",
				icon: Clock,
			},
			{
				...
				trend: "up",
				status:
					analytics.performance.onTimeFilings >= 95
						? "excellent"
						: analytics.performance.onTimeFilings >= 85
							? "good"
							: "warning",
				icon: Calendar,
			},
			{
				...
				trend: "up",
				status:
					(analytics.satisfactionScore || 85) >= 90
						? "excellent"
						: (analytics.satisfactionScore || 85) >= 80
							? "good"
							: "warning",
				icon: Star,
			},
			{
				...
				status:
					analytics.outstandingFees === 0
						? "excellent"
						: analytics.outstandingFees < 1000
							? "good"
							: "warning",
				icon: Target,
			},
	...
	const timeSeriesData = useMemo(() => {
		...
		return months.slice(0, 6).map((month, index) => ({
			...
			compliance: Math.max(
				60,
				analytics.complianceScore - 10 + Math.random() * 20,
			),
			filings: Math.max(
				5,
				analytics.totalFilings * (0.8 + Math.random() * 0.4),
			),
			responseTime: Math.max(
				1,
				analytics.averageResponseTime * (0.8 + Math.random() * 0.4),
			),
		}));
	}, [analytics]);
		...
		return [
			{
				...
				maxScore: 100,
				status:
					analytics.riskAssessment.score >= 80
						? "low"
						: analytics.riskAssessment.score >= 60
							? "medium"
							: "high",
			},
			{
			...
			},
			{
				...
				maxScore: 100,
				status:
					analytics.performance.serviceQuality >= 80
						? "low"
						: analytics.performance.serviceQuality >= 60
							? "medium"
							: "high",
			},
		];
								...
								<CardContent className="p-6">
									<div className="flex items-center justify-between">
										<div className="space-y-2">
											...
											<div className="flex items-center space-x-2">
												<TrendIcon
													className={cn(
														"h-4 w-4",
														trend.isPositive && metric.trend === "up"
															? "text-green-600"
															: trend.isPositive && metric.trend === "down"
																? "text-red-600"
																: !trend.isPositive && metric.trend === "up"
																	? "text-red-600"
																	: "text-green-600",
										...
										<div className="flex flex-col items-end space-y-2">
											<Icon
												className={cn("h-8 w-8", getStatusColor(metric.status))}
											/>
											<Badge
												variant={
									...
									<div key={risk.category}>
										...
										<div className="space-y-2">
											...
											<div className="flex justify-between text-muted-foreground text-xs">
												<span>
													{risk.score}/{risk.maxScore}
												</span>
												<span>
													{((risk.score / risk.maxScore) * 100).toFixed(0)}%
												</span>
											</div>
										</div>
									</div>
						...
						<div className="grid grid-cols-1 gap-6 md:grid-cols-2">
							<div>
								...
								<ul className="space-y-1 text-muted-foreground text-sm">
									{analytics.complianceScore >= 80 && (
										<li>
											• High compliance score indicates strong regulatory
											adherence
										</li>
									)}
									...
									{analytics.outstandingFees === 0 && (
										<li>
											• No outstanding fees shows good financial management
										</li>
									)}
								</ul>
							</div>
							<div>
								...
								<ul className="space-y-1 text-muted-foreground text-sm">
									{analytics.complianceScore < 80 && (
										<li>
											• Compliance score could be improved with better
											documentation
										</li>
									)}
									{analytics.averageResponseTime > 5 && (
										<li>
											• Response time should be reduced for better service
										</li>
									)}
									{analytics.outstandingFees > 0 && (
