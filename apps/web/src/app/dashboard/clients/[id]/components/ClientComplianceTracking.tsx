...
export function ClientComplianceTracking({
	...
	): keyof typeof COMPLIANCE_COLORS => {
		switch (level) {
			case "green":
				return "green";
			case "amber":
	...
	const calculateOverallScore = (): number => {
		const totalScore = complianceCategories.reduce(
			(sum, cat) => sum + cat.score,
			0,
		);
		const totalMaxScore = complianceCategories.reduce(
				...
				<div className="flex flex-col space-y-4 md:flex-row md:items-center md:justify-between md:space-y-0">
					<div>
						...
						<p className="text-muted-foreground text-sm">
							Monitor compliance status with visual progress indicators and
							alerts
						</p>
					</div>
						...
						<CardTitle className="flex items-center justify-between">
							...
							<Badge
								...
								className={cn(
									...
									COMPLIANCE_COLORS[getComplianceLevelColor(currentLevel)]
										.border,
								)}
							>
