...
function DraggableField({
	...
	return (
		<div
			...
			{...listeners}
			{...attributes}
			className={`relative cursor-pointer rounded-lg border p-3 transition-all ${isDragging ? "scale-95 opacity-50" : ""}
        ${readonly ? "cursor-not-allowed opacity-50" : "hover:border-blue-300 hover:shadow-md"}bg-white border-gray-200`}
			onClick={() => !readonly && onAddField(fieldType.type)}
		>
			<div className="flex items-start space-x-3">
				<div
					className={`flex-shrink-0 rounded-md p-2 ${
						fieldType.category === "basic"
							? "bg-blue-100 text-blue-600"
							: fieldType.category === "advanced"
								? "bg-purple-100 text-purple-600"
								: fieldType.category === "guyanese"
	...
	return (
		<div className="space-y-6 p-4">
			...
			{categories.map((category) => {
				const fields = FIELD_TYPES.filter(
			...
			<Card>
				...
				<CardContent className="space-y-2 text-gray-600 text-xs">
