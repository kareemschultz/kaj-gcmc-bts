...
import { FormValidationEngine, createValidationEngine } from "@/lib/form-builder/validation";
import { CalculationEngine, createCalculationEngine } from "@/lib/form-builder/calculations";
...
export function FormRenderer({
    ...
    configuration.fields.forEach(field => {
      ...
      switch (field.type) {
        ...
        case "radio":
          ...
          const options = selectField.options?.map((opt: any) => opt.value) || [];
          schemaFields[field.name] = field.required
            ? z.enum(options.length > 0 ? [options[0], ...options.slice(1)] : [""])
            : z.enum(options.length > 0 ? [options[0], ...options.slice(1)] : [""]).optional();
          break;
        case "checkbox":
          schemaFields[field.name] = field.required
        ...
        default:
          schemaFields[field.name] = field.required
            ? z.unknown().refine(val => val !== null && val !== undefined, "This field is required")
            : z.unknown().optional();
      }
    });
  ...
  const { control, handleSubmit, setValue, getValues, formState: { errors, isDirty, isValid } } = form;
  ...
  const progress = useMemo(() => {
    if (!isMultiStep) {
      ...
      const completedFields = configuration.fields.filter(f =>
        f.required && watchedValues[f.name] !== undefined && watchedValues[f.name] !== ""
      ).length;
  ...
  }, [isMultiStep, configuration.fields, configuration.steps, completedSteps, currentStepIndex, watchedValues]);
  ...
  const handleFieldChange = useCallback((fieldName: string, value: unknown) => {
    ...
    const fieldErrors = validationEngine.validateFieldRealTime(fieldName, value, getValues());
    setFieldErrors(prev => ({
      ...prev,
  ...
  }, [setValue, validationEngine, getValues, onFieldChange, configuration.fields]);

  // Update calculated fields
  const updateCalculatedFields = useCallback((changedField: string, changedValue: unknown) => {
    ...
    const calculationEngine = createCalculationEngine(formData, configuration.fields);
    ...
    const newCalculatedValues: Record<string, unknown> = { ...calculatedValues };

    dependentFields.forEach(field => {
      if (field.type === "calculated" && field.calculation) {
        try {
          const calculatedValue = calculationEngine.calculateField(field.calculation);
  ...
  const goToNextStep = useCallback(() => {
    if (!configuration.steps || currentStepIndex >= configuration.steps.length - 1) return;

    // Validate current step
    const currentStepFields = getCurrentStepFields();
    const hasErrors = currentStepFields.some(field => fieldErrors[field.name]?.length > 0);
  ...
  const goToStep = useCallback((stepIndex: number) => {
    if (stepIndex < 0 || !configuration.steps || stepIndex >= configuration.steps.length) return;
    ...
    <div className={`form-renderer ${className}`}>
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">{configuration.title}</h2>
            {configuration.description && (
              <p className="text-gray-600 mt-1">{configuration.description}</p>
            )}
          </div>
        ...
        <div className="mt-4">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-gray-700">
              {isMultiStep ? `Step ${currentStepIndex + 1} of ${configuration.steps?.length}` : "Progress"}
            </span>
        ...
        <Alert className="mb-6 border-red-200 bg-red-50">
          ...
          <AlertDescription className="text-red-800">
            <p className="font-semibold mb-2">
              Please fix {validationSummary.errorCount} error(s) before submitting:
            </p>
      ...
      <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6">
            ...
            {currentSections.map(section => (
              <Card key={section.id} className="mb-6">
                ...
                <CardContent className="space-y-6">
                  {section.fields
                    .map(fieldId => configuration.fields.find(f => f.id === fieldId))
                    .filter(Boolean)
                        ...
                        <Controller
                          ...
                          render={({ field: formField, fieldState }) => (
                            <FormFieldRenderer
                              ...
                              error={fieldState.error?.message || fieldErrors[field!.name]?.[0]?.message}
        ...
        <div className="flex items-center justify-between pt-6 border-t">
          ...
          <div className="flex items-center gap-3">
            {isMultiStep && currentStepIndex < (configuration.steps?.length || 1) - 1 ? (
              <Button
                type="button"
