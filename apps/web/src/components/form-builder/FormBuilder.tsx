...
function formBuilderReducer(state: FormBuilderState, action: FormBuilderAction): FormBuilderState {
  switch (action.type) {
    ...
    case "ADD_FIELD":
      ...
      const targetIndex = action.payload.targetIndex ?? state.form.fields.length;
    ...
    case "REMOVE_FIELD":
      ...
      return {
        ...
        form: {
          ...state.form,
          fields: state.form.fields.filter(field => field.id !== fieldToRemove),
          sections: state.form.sections.map(section => ({
            ...section,
            fields: section.fields.filter(fieldId => fieldId !== fieldToRemove)
          }))
        },
        selectedField: state.selectedField === fieldToRemove ? undefined : state.selectedField,
        isDirty: true
      };
    ...
    case "MOVE_FIELD":
      ...
      let movedFields = state.form.fields.filter(f => f.id !== fieldId);
    ...
    case "ADD_SECTION":
      ...
      const sectionTargetIndex = action.payload.targetIndex ?? state.form.sections.length;
    ...
    case "REMOVE_SECTION":
      return {
        ...
        form: {
          ...state.form,
          sections: state.form.sections.filter(section => section.id !== action.payload.sectionId)
        },
        selectedSection: state.selectedSection === action.payload.sectionId ? undefined : state.selectedSection,
        isDirty: true
      };
...
export function FormBuilder({
  ...
  React.useEffect(() => {
    if (initialConfiguration) {
      dispatch({
        type: "SET_FORM",
        payload: { ...createInitialState().form, ...initialConfiguration } as FormConfiguration
      });
    }
  }, [initialConfiguration]);
  ...
  // Add new field
  const handleAddField = useCallback((fieldType: FormFieldType, targetIndex?: number) => {
  ...
  const handleDuplicateField = useCallback((fieldId: string) => {
    ...
    const originalIndex = state.form.fields.findIndex(f => f.id === fieldId);
    dispatch({
      type: "ADD_FIELD",
  ...
  const validationResults = useMemo(() => {
    ...
    state.form.fields.forEach(field => {
      if (field.required && !field.validationRules?.some(rule => rule.type === "required")) {
        warnings.push(`Required field "${field.label}" should have required validation rule`);
      }
    });
  ...
  React.useEffect(() => {
    dispatch({ type: "SET_ERRORS", payload: { errors: validationResults.errors } });
    dispatch({ type: "SET_WARNINGS", payload: { warnings: validationResults.warnings } });
  }, [validationResults]);
  ...
  return (
    <div className={`form-builder h-screen flex flex-col ${className}`}>
      {/* Header */}
      <div className="border-b bg-white px-6 py-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            {/* Mode toggle */}
            <Tabs value={state.mode} onValueChange={(mode) => dispatch({ type: "SET_MODE", payload: { mode: mode as any } })}>
              <TabsList>
            ...
            {!readonly && (
              <>
                <Button variant="outline" onClick={handleSave} disabled={isLoading || !state.isDirty}>
                      ...
                      <div className="grid grid-cols-3 gap-4">
                        <div>
                          ...
                          <Select
                            ...
                            onValueChange={(value: Authority) => dispatch({
                              type: "UPDATE_FORM",
                        ...
                        <div>
                          ...
                          <Select
                            ...
                            onValueChange={(value: DocumentCategory) => dispatch({
                              type: "UPDATE_FORM",
                  ...
                  <Card>
                    ...
                    <CardContent>
                      {state.form.fields.length === 0 ? (
                        ...
                        <SortableContext items={state.form.fields.map(f => f.id)} strategy={verticalListSortingStrategy}>
                          <div className="space-y-3">
                            {state.form.fields.map((field, index) => (
                              <SortableField
                                ...
                                onSelect={() => dispatch({ type: "SELECT_FIELD", payload: { fieldId: field.id } })}
                ...
                <DragOverlay>
                  {state.draggedField && (
                    <div className="bg-white border rounded-lg shadow-lg p-3">
                      <span className="font-medium">{state.draggedField.label}</span>
                    </div>
                  )}
                </DragOverlay>
              </DndContext>
