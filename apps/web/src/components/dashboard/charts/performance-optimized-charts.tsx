...
function useDataVirtualization<T>(data: T[], config: VirtualizationConfig) {
	...
	const loadChunks = useCallback(
		async (startChunk: number, endChunk: number) => {
	...
	const navigateToChunk = useCallback(
		(chunkIndex: number) => {
			...
			const endChunk = Math.min(
				totalChunks - 1,
				chunkIndex + config.preloadChunks,
			);
...
function usePerformanceMonitor() {
	...
	const endTimer = useCallback((type: "render" | "data") => {
		...
		setMetrics((prev) => ({
			...
			reRenderCount:
				type === "render" ? prev.reRenderCount + 1 : prev.reRenderCount,
		}));
	}, []);
...
function VirtualDataChart({
	...
	const shouldVirtualize =
		config.enabled && data.length > config.recycleThreshold;
...
function PerformanceMonitor({
	...
	return (
		<motion.div
			...
			className="fixed right-4 bottom-4 z-50 overflow-hidden rounded-lg border bg-white shadow-lg"
		>
			<div
				className="flex cursor-pointer items-center justify-between bg-gray-50 p-2"
				onClick={onToggle}
			>
				<div className="flex items-center gap-2">
			...
			<AnimatePresence>
				{visible && (
					<motion.div
						...
						animate={{ opacity: 1 }}
						exit={{ opacity: 0 }}
						className="min-w-[250px] space-y-3 p-3"
					>
		...
		<div className="space-y-6">
			<div className="border-gray-200 border-b pb-4">
				...
				<p className="mt-2 text-gray-600">
					Advanced optimization features: lazy loading, virtualization, and
