		...
		case "pieChart":
			if (Array.isArray(apiData)) {
				return apiData.map((item) => ({
					name: item._count
						? `${item.status || item.level || item.type}`
						: item.name,
			...
			if (apiData.activity) {
				return [
					...
					{ name: "New Filings", value: apiData.activity.newFilings || 0 },
					{
						...
						value: apiData.activity.newServiceRequests || 0,
					},
					{
						name: "Completed Tasks",
						value: apiData.activity.completedTasks || 0,
					},
				];
			}
			return [];
		...
		case "complianceRadial":
			if (apiData.byLevel) {
				return [
					{
						name: "High Risk",
						value: apiData.byLevel.high || 0,
						color: "#EF4444",
					},
					{
						name: "Medium Risk",
						value: apiData.byLevel.medium || 0,
						color: "#F59E0B",
					},
					{
						name: "Low Risk",
						value: apiData.byLevel.low || 0,
						color: "#10B981",
					},
				];
			}
						...
						{showDetails && (
							...
							>
								<div className="grid grid-cols-2 gap-4 text-sm md:grid-cols-4">
									<div>
										<div className="mb-1 font-medium">Dashboard Data</div>
									...
									<div>
										<div className="mb-1 font-medium">Compliance Data</div>
										<div className="text-muted-foreground">
											<div>Overview: ✓</div>
									...
									<div>
										<div className="mb-1 font-medium">Task Data</div>
										<div className="text-muted-foreground">
											<div>Overview: ✓</div>
											<div>Progress: ✓</div>
											<div>Assignments: ✓</div>
									...
									<div>
										<div className="mb-1 font-medium">Service Data</div>
										<div className="text-muted-foreground">
											<div>Pipeline: ✓</div>
											<div>Requests: ✓</div>
											<div>Analytics: ✓</div>
								...
								{error && (
									<Alert className="mt-4">
										<AlertTriangle className="h-4 w-4" />
										<AlertDescription>
											{error}. The system will retry automatically, or you can
											refresh manually.
										</AlertDescription>
									</Alert>
								)}
