...
export interface ExecutiveSummaryData {
  ...
  monthlyTrends: { month: string; revenue: number; clients: number; satisfaction: number }[];
}

export interface ComplianceHealthData {
  overallScore: number;
  agencyScores: { agency: string; score: number; target: number; status: 'good' | 'warning' | 'critical' }[];
...
export interface FinancialPerformanceData {
  ...
  cashFlow: { month: string; inflow: number; outflow: number; netFlow: number }[];
  budgetVariance: { category: string; budget: number; actual: number; variance: number }[];
}

export interface ClientPortfolioData {
  segmentation: { segment: string; count: number; revenue: number; riskLevel: string }[];
  acquisitionTrends: { month: string; newClients: number; churnRate: number; retentionRate: number }[];
  clientValue: { tier: string; count: number; avgValue: number; lifetimeValue: number }[];
  satisfactionScores: { department: string; score: number; trend: number }[];
}

export interface OperationalEfficiencyData {
  processingTimes: { process: string; current: number; target: number; improvement: number }[];
  staffProductivity: { department: string; productivity: number; utilization: number; efficiency: number }[];
  serviceMetrics: { metric: string; value: number; target: number; status: string }[];
  resourceUtilization: { resource: string; utilization: number; capacity: number; availability: number }[];
}
...
const generateFinancialPerformance = (): FinancialPerformanceData => ({
  ...
  budgetVariance: [
    ...
    { category: 'Office Expenses', budget: 78000, actual: 81000, variance: 3.8 },
  ],
});
        ...
        <CardContent>
          <div className="flex items-center justify-center space-x-8">
            <div className="text-center">
              ...
              <Progress value={data.overallScore} className="w-32 mt-2" />
            </div>
            <div className="space-y-3">
              {data.agencyScores.map((agency, index) => (
                <div key={index} className="flex items-center gap-3">
                  <Badge variant={
                    agency.status === 'good' ? 'default' :
                    agency.status === 'warning' ? 'secondary' : 'destructive'
                  }>
                    {agency.agency}
                  </Badge>
                  <div className="w-24">
              ...
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={data.complianceTrends}>
                  ...
                  <Bar
                    ...
                    name="Incidents"
                  />
                  <ReferenceLine yAxisId="left" y={90} stroke="#10B981" strokeDasharray="3 3" />
                </ComposedChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
...
export function BusinessIntelligenceDashboard() {
  ...
  const financialPerformance = useMemo(() => generateFinancialPerformance(), []);
    ...
    >
      <div className="border-b border-gray-200 pb-4">
        ...
        <p className="text-gray-600 mt-2">
          Executive-level analytics with key performance indicators and strategic insights
        </p>
      </div>
