...
const generateGanttData = (): GanttData => ({
  ...
  milestones: [
    { id: 'm1', name: 'Q1 Filing Deadline', date: new Date('2024-03-15'), completed: false },
    { id: 'm2', name: 'Audit Completion', date: new Date('2024-04-30'), completed: false },
    { id: 'm3', name: 'System Upgrade', date: new Date('2024-06-01'), completed: false },
  ]
});
...
const generateFunnelData = (): FunnelData => ({
  stages: [
    { name: 'Website Visitors', value: 10000, percentage: 100, color: '#3B82F6' },
    { name: 'Service Inquiries', value: 2500, percentage: 25, color: '#10B981' },
    { name: 'Consultations Scheduled', value: 1200, percentage: 12, color: '#F59E0B' },
...
export function GanttChart({ data, height = 400 }: { data: GanttData; height?: number }) {
  ...
  const getTaskPosition = (task: GanttData['tasks'][0]) => {
    const startPos = ((task.startDate.getTime() - timelineStart.getTime()) / timelineWidth) * 100;
    const duration = ((task.endDate.getTime() - task.startDate.getTime()) / timelineWidth) * 100;
    ...
    <Card>
      ...
      <CardContent>
        <div style={{ height }} className="relative overflow-x-auto">
          {/* Timeline header */}
          <div className="mb-4 relative h-8 bg-gray-100 rounded">
            {['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun'].map((month, index) => (
              <div
                key={month}
                className="absolute top-0 h-full flex items-center justify-center border-r border-gray-300"
                style={{
                  left: `${(index * 100) / 6}%`,
                  width: `${100 / 6}%`,
                }}
            ...
            {data.tasks.map((task) => {
              ...
              return (
                <div key={task.id} className="relative h-12">
                  {/* Task name */}
                  <div className="absolute left-0 w-48 h-full flex items-center">
                  ...
                  <div
                    className={cn(
                      "absolute h-8 rounded cursor-pointer transition-all duration-200",
          ...
          <div className="mt-6 relative h-8">
            {data.milestones.map((milestone) => {
              const pos = ((milestone.date.getTime() - timelineStart.getTime()) / timelineWidth) * 100;
              return (
                <div
                  key={milestone.id}
                ...
                >
                  <div className={cn(
                    "w-3 h-3 rounded-full border-2 transform -translate-x-1",
                    milestone.completed ? "bg-green-500 border-green-600" : "bg-yellow-500 border-yellow-600"
                  )} />
...
export function HeatMapChart({ data, height = 400 }: { data: HeatMapData; height?: number }) {
    ...
    <Card>
      ...
      <CardContent>
        <div style={{ height }} className="relative">
          {/* Y-axis labels */}
          <div className="absolute left-0 top-8 bottom-8">
          ...
          {/* X-axis labels */}
          <div className="absolute left-16 bottom-0 right-0 h-8">
            {data.xLabels.filter((_, i) => i % 2 === 0).map((label, index) => (
              <div
                key={label}
          ...
          {/* Heat map grid */}
          <div className="absolute left-16 top-8 bottom-8 right-0">
            {data.data.map((cell, index) => {
              ...
              return (
                <div
                  key={index}
                  className={cn(
                    "absolute border border-gray-200 cursor-pointer transition-all duration-200",
...
export function GaugeChart({ data, size = 200 }: { data: GaugeData; size?: number }) {
        ...
        <div className="relative" style={{ width: size, height: size / 2 + 40 }}>
          ...
          >
            ...
            <line
              ...
              y2={size / 2 + 20 + Math.sin((angle * Math.PI) / 180) * (size / 2 - 30)}
              stroke="#374151"
              strokeWidth="3"
              strokeLinecap="round"
              className="transition-all duration-1000"
      ...
      <CardContent>
        <div className="space-y-4">
          {data.stages.map((stage, index) => {
            ...
            const conversionRate = prevStage ? ((stage.value / prevStage.value) * 100).toFixed(1) : '100.0';
            ...
            return (
              <div key={stage.name} className="space-y-2">
                ...
                <div
                  className="h-12 rounded flex items-center justify-center text-white font-medium transition-all duration-700"
                  style={{
                    width: `${width}%`,
  ...
  return (
    <div className="space-y-8">
      <div className="border-b border-gray-200 pb-4">
        <h1 className="text-2xl font-bold text-gray-900">Advanced Chart Types</h1>
        <p className="text-gray-600 mt-2">
          Specialized visualizations for complex data analysis and project management
        </p>
      </div>
