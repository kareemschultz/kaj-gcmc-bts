...
function MobileOptimizedChart({
    ...
    if (active && payload && payload.length) {
      return (
        <motion.div
          ...
          className={cn(
            "bg-black/80 text-white rounded-lg p-3 shadow-xl",
            "max-w-xs text-sm",
        ...
        return (
          <LineChart {...commonProps}>
            ...
            <Line
              ...
              dot={{ fill: '#3B82F6', strokeWidth: 2, r: touchSupported ? 6 : 4 }}
        ...
        return (
          <PieChart>
            <Pie
              ...
              dataKey="value"
              label={({ name, percent }) =>
                deviceType !== 'mobile' ? `${name} ${(percent * 100).toFixed(0)}%` : `${(percent * 100).toFixed(0)}%`
              }
            >
  ...
  return (
    ...
    >
      <Card className={cn(
        "overflow-hidden",
        touchSupported && "shadow-lg"
      )}>
        <CardHeader className={cn(
          "pb-2",
          deviceType === 'mobile' && "pb-1"
        )}>
    ...
    <div className={cn("space-y-4", className)}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className={cn(
            "p-2 rounded-lg",
            deviceType === 'mobile' ? "bg-green-50 text-green-600" :
            deviceType === 'tablet' ? "bg-blue-50 text-blue-600" :
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {deviceType} • {orientation} • {touchSupported ? 'Touch enabled' : 'Mouse only'}
            </p>
          </div>
        </div>
        ...
        <div className={cn(
          ...
          {chartsToRender.map((chart, index) => (
            <MobileOptimizedChart
              ...
              config={chart}
              className={cn(
                index === currentChart && "ring-2 ring-blue-500"
              )}
            />
          ))}
        </div>
