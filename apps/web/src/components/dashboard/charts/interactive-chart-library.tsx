...
export type ChartType = 'line' | 'area' | 'bar' | 'pie' | 'radial' | 'scatter' | 'treemap' | 'funnel' | 'gantt' | 'heatmap' | 'gauge' | 'sankey';
  ...
  if (active && payload && payload.length) {
    return (
      <motion.div
        ...
        className="bg-white/95 backdrop-blur-md border border-gray-200 rounded-lg shadow-xl p-4 min-w-[200px]"
      >
  ...
  <div className="flex items-center gap-2">
    ...
    {config.exportable && (
      <div className="relative group">
        ...
        <div className="absolute right-0 top-8 bg-white border border-gray-200 rounded-lg shadow-lg opacity-0 group-hover:opacity-100 transition-opacity z-50">
          <div className="py-1">
            <button
...
const useRealTimeData = (initialData: ChartData[], interval: number = 5000) => {
        ...
        <LineChart {...commonProps}>
          ...
          {config.dataKeys.slice(1).map((key, index) => (
            <Line
              ...
              strokeWidth={2}
              dot={{ fill: config.colors[index] || '#3B82F6', strokeWidth: 2, r: 4 }}
              activeDot={{ r: 6, stroke: config.colors[index] || '#3B82F6', strokeWidth: 2 }}
              onClick={handleClick}
            />
          ))}
        </LineChart>
          ...
          <defs>
            {config.dataKeys.slice(1).map((key, index) => (
              <linearGradient key={key} id={`gradient-${key}`} x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%" stopColor={config.colors[index] || '#3B82F6'} stopOpacity={0.3}/>
                <stop offset="95%" stopColor={config.colors[index] || '#3B82F6'} stopOpacity={0.01}/>
              </linearGradient>
            ))}
          </defs>
          <CartesianGrid strokeDasharray="3 3" opacity={0.3} />
        ...
        <PieChart>
          ...
          >
            {data.map((entry, index) => (
              <Cell key={`cell-${index}`} fill={config.colors[index % config.colors.length] || '#3B82F6'} />
            ))}
          </Pie>
          <Tooltip content={<CustomTooltip />} />
          <Legend />
...
export function InteractiveChart({
  ...
  const handleExport = useCallback(async (format: 'pdf' | 'excel' | 'png') => {
      ...
      >
        ...
        <CardContent>
          <div
            ...
            style={{ height: isFullscreen ? 'calc(100vh - 200px)' : chartHeight }}
          >
