		...
		questions: [
			{
				...
				options: [
					...
					{
						value: "mostly",
						label: "Mostly current (1-2 periods behind)",
						score: 15,
					},
			...
			},
			{
				...
				options: [
					...
					{
						value: "mostly",
						label: "Minor arrears (under 30 days)",
						score: 15,
					},
					{
						value: "behind",
						label: "Significant arrears (over 30 days)",
						score: 5,
					},
				...
				options: [
					{
						...
						label: "Registered and current",
						score: 15,
					},
					{
						value: "registered_behind",
						label: "Registered but behind",
						score: 8,
					},
					{
						value: "should_register",
						label: "Should be registered but not",
						score: 2,
					},
		...
		questions: [
			...
			},
			{
				...
				options: [
					...
					{
						value: "expiring",
						label: "Expiring soon (within 60 days)",
						score: 15,
					},
		...
		questions: [
			...
			},
			{
				...
				options: [
					...
					{
						value: "minor_arrears",
						label: "Minor arrears (1-2 months)",
						score: 15,
					},
					{
						value: "major_arrears",
						label: "Major arrears (3+ months)",
						score: 5,
					},
				...
				<div className="grid grid-cols-1 gap-4 md:grid-cols-2">
					<div>
						...
						<Input
							...
							onChange={(e) =>
								handleChange("employees", Number.parseInt(e.target.value) || 0)
							}
							className="mt-2"
						/>
					</div>
...
function AssessmentStep() {
	...
	const currentArea =
		complianceAreas.find((area) => area.id === activeArea) ||
		complianceAreas[0];
		...
		<div className="space-y-6">
							...
							{complianceAreas.map((area) => (
								<div key={area.id} className="text-center">
									<div className="font-semibold text-lg">
										{getAreaScore(area.id)}%
									</div>
									<div className="text-gray-600 text-sm">{area.name}</div>
								</div>
							))}
						</div>
					</div>
			...
			<Card>
				<CardHeader>
					...
					<p className="text-gray-600 text-sm">{currentArea.description}</p>
				</CardHeader>
...
function ResultsStep() {
	...
	const calculateResults = useMemo(() => {
		...
		const overallMaxScore = results.reduce(
	...
	const getRecommendations = () => {
		...
		calculateResults.areas.forEach((area) => {
			if (area.criticalIssues > 0) {
				recs.push(
					`Address ${area.criticalIssues} critical issue(s) in ${area.name}`,
				);
			}
			if (area.percentage < 60) {
				recs.push(
					`Improve overall compliance in ${area.name} (currently ${area.percentage}%)`,
				);
			}
		});
			...
			<Card className="border-2">
				...
				<CardContent>
					<div className="space-y-4 text-center">
						<div>
							<div className="mb-2 font-bold text-6xl text-blue-600">
						...
						<div
							className={`inline-flex items-center gap-2 rounded-full border px-4 py-2 ${getStatusColor(
								calculateResults.overall >= 80
									? "excellent"
									: calculateResults.overall >= 60
										? "good"
										: calculateResults.overall >= 40
											? "fair"
											: "poor",
							)}`}
						>
					...
					<Card key={area.id} className="border">
						...
						<CardContent className="space-y-3">
							<div className="flex items-center justify-between">
								<span className="font-bold text-2xl">{area.percentage}%</span>
					...
					<div className="grid grid-cols-2 gap-4 text-sm md:grid-cols-4">
						...
						<div>
							...
							<div className="font-medium">
								{businessData.businessType?.replace("_", " ")}
							</div>
						</div>
						<div>
