...
const filingSelectionSchema = z.object({
	selectedFilings: z
		.array(z.string())
		.min(1, "Please select at least one filing"),
...
const documentsSchema = z.object({
	...
	documentsComplete: z
		.boolean()
		.refine((val) => val, "Please confirm documents are complete"),
});

const reviewSchema = z.object({
	dataAccurate: z
		.boolean()
		.refine((val) => val, "Please confirm data accuracy"),
	authorizedToFile: z.boolean().refine((val) => val, "Authorization required"),
	understoodDeadlines: z
		.boolean()
		.refine((val) => val, "Please acknowledge deadlines"),
...
const submissionSchema = z.object({
	...
	deliveryMethod: z
		.enum(["electronic", "mail", "pickup", "courier"])
		.optional(),
				...
				<CardContent className="space-y-4">
					<div className="grid grid-cols-1 gap-4 md:grid-cols-2">
						<div>
							...
							<Select
								value={data.taxYear || ""}
								onValueChange={(value) => handleChange("taxYear", value)}
							>
					...
					<div>
						<Label htmlFor="filingPeriod">
							Specific Filing Period (if applicable)
						</Label>
						<Input
							id="filingPeriod"
...
function DocumentsStep() {
	...
	const handleFileUpload = (files: FileList | null) => {
		if (files) {
			...
			setData({
				...data,
				additionalDocuments: [...uploadedFiles, ...fileNames],
			});
		}
	};
								...
								>
									<Checkbox
										...
										onCheckedChange={(checked) =>
											handleChecklistChange(document, checked as boolean)
										}
									/>
									<div className="flex-1">
										<Label className="font-medium text-sm">{document}</Label>
										<div className="mt-1 text-gray-500 text-xs">
											Required for:{" "}
											{filingData.selectedFilings
												?.filter((id: string) => {
													const filing = availableFilings.find(
														(f) => f.id === id,
													);
												...
												.map(
													(id: string) =>
														availableFilings.find((f) => f.id === id)?.name,
												)
												.join(", ")}
										</div>
									</div>
						...
						<div className="mt-6">
							<div className="mb-4 flex items-center space-x-2">
								<Checkbox
									...
									onCheckedChange={(checked) =>
										handleChange("documentsComplete", checked)
									}
								/>
								<Label className="font-medium">
									I confirm that all required documents are available and
									complete
								</Label>
							</div>
						</div>
		...
		<div className="space-y-6">
			<Card>
				...
				<CardContent className="space-y-4">
					...
					<div>
						<h4 className="mb-2 font-medium">Selected Filings:</h4>
						<ul className="space-y-1">
					...
					<div>
						<h4 className="mb-2 font-medium">Document Status:</h4>
								...
								{documentsData.documentsComplete ? (
									<>
										...
										<span className="text-green-700">
											All documents confirmed complete
										</span>
									</>
								) : (
									<>
										...
										<span className="text-yellow-700">
											Documents pending confirmation
										</span>
									</>
								)}
							</div>
				...
				<CardContent className="space-y-4">
					<div className="space-y-4">
						<div className="flex items-start space-x-3">
							<Checkbox
								...
								onCheckedChange={(checked) =>
									handleChange("dataAccurate", checked as boolean)
								}
							/>
							<div>
								<Label>
								...
								<p className="mt-1 text-gray-600 text-sm">
									All data and documents provided are true and complete to the
						...
						<div className="flex items-start space-x-3">
							<Checkbox
								...
								onCheckedChange={(checked) =>
									handleChange("authorizedToFile", checked as boolean)
								}
							/>
							<div>
								<Label>
									I am authorized to submit these filings on behalf of the
									entity
								</Label>
								<p className="mt-1 text-gray-600 text-sm">
									I have the legal authority to file these returns and make
									declarations
								</p>
							</div>
						</div>
						...
						<div className="flex items-start space-x-3">
							<Checkbox
								...
								onCheckedChange={(checked) =>
									handleChange("understoodDeadlines", checked as boolean)
								}
							/>
							<div>
								<Label>
									I understand the filing deadlines and consequences of late
									filing
								</Label>
								<p className="mt-1 text-gray-600 text-sm">
									I acknowledge the deadlines and potential penalties for late
						...
						<div className="flex items-start space-x-3">
							...
							<div>
								<Label>
									I agree to the terms of service and professional engagement
								</Label>
								<p className="mt-1 text-gray-600 text-sm">
					...
					<div className="mt-6 rounded-lg border border-yellow-200 bg-yellow-50 p-4">
						...
						<ul className="space-y-1 text-sm text-yellow-800">
							...
							<li>
								• You will receive confirmation once filings are submitted
							</li>
				...
				<CardContent className="space-y-4">
					<div>
						...
						<Select
							...
							onValueChange={(value) => handleChange("submissionMethod", value)}
						>
							<SelectTrigger className="mt-2">
					...
					<div className="grid grid-cols-1 gap-4 md:grid-cols-2">
						<div>
							<Label htmlFor="contactForQuestions">
							...
							<Input
								...
								onChange={(e) =>
									handleChange("contactForQuestions", e.target.value)
								}
								className="mt-2"
							/>
						</div>
					...
					<div className="space-y-4">
						<div className="rounded-lg border border-blue-200 bg-blue-50 p-4">
							...
							<ul className="space-y-1 text-blue-800 text-sm">
								...
								<li>
									• You'll receive confirmation when filings are submitted to
									authorities
								</li>
