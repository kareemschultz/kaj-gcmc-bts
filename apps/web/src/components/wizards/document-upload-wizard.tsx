...
const documentTypeSchema = z.object({
  ...
  retentionPeriod: z.enum(["1_year", "3_years", "7_years", "permanent"]).default("7_years"),
});
...
const reviewSchema = z.object({
  confirmAccuracy: z.boolean().refine((val) => val, "Please confirm document accuracy"),
  confirmCompliance: z.boolean().refine((val) => val, "Please confirm compliance"),
...
function DocumentTypeStep() {
  ...
  const categoryOptions = [
    {
      ...
      subcategories: ["Income Tax Returns", "VAT Returns", "Withholding Tax", "Tax Assessments", "GRA Correspondence"]
    },
    {
      value: "business_registration",
      label: "Business Registration",
      description: "Incorporation documents, licenses",
      subcategories: ["Certificate of Incorporation", "Business License", "Trade License", "Professional License"]
    },
    {
      value: "financial_statements",
      label: "Financial Statements",
      description: "Audited accounts, management reports",
      subcategories: ["Audited Financial Statements", "Management Accounts", "Bank Statements", "Trial Balance"]
    },
    {
      value: "compliance_certificates",
      label: "Compliance Certificates",
      description: "Regulatory compliance documents",
      subcategories: ["NIS Compliance", "EPA Permits", "Health Certificates", "Safety Certificates"]
    },
    {
      value: "legal_documents",
      label: "Legal Documents",
      description: "Contracts, agreements, legal notices",
      subcategories: ["Contracts", "Agreements", "Legal Notices", "Court Documents"]
    },
    {
      value: "other",
  ...
  const selectedCategory = categoryOptions.find(cat => cat.value === data.category);
          ...
          <div className="space-y-4">
            ...
            {selectedCategory && (
              <div>
                ...
                <Select value={data.subcategory || ""} onValueChange={(value) => handleChange("subcategory", value)}>
                  <SelectTrigger className="mt-2">
            ...
            <div className="space-y-4">
              <div className="flex items-start space-x-3">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => handleChange("isConfidential", checked)}
                />
                <div>
              ...
              <div>
                ...
                <Select value={data.retentionPeriod || "7_years"} onValueChange={(value) => handleChange("retentionPeriod", value)}>
                  <SelectTrigger className="mt-2">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
            ...
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center hover:border-gray-400 transition-colors">
              ...
              <div className="mt-4">
                ...
                <p className="text-xs text-gray-400 mt-1">
                  Supported formats: PDF, JPG, PNG, DOC, DOCX, XLS, XLSX (max 50MB each)
                </p>
              </div>
            </div>
            ...
            {uploadedFiles.length > 0 && (
              <Card className="border border-green-200 bg-green-50">
                <CardHeader className="pb-3">
                  <CardTitle className="text-base text-green-900">Uploaded Files</CardTitle>
                </CardHeader>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <Select value={data.relatedClient || ""} onValueChange={(value) => handleChange("relatedClient", value)}>
                <SelectTrigger className="mt-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                {[
                  ...
                  <div key={option} className="flex items-center space-x-2">
                    <Checkbox
                      checked={data.authorizedViewers?.includes(option) || false}
                      onCheckedChange={(checked) => {
                        const current = data.authorizedViewers || [];
                        if (checked) {
                          handleChange("authorizedViewers", [...current, option]);
                        } else {
                          handleChange("authorizedViewers", current.filter(v => v !== option));
                        }
                      }}
                    />
        ...
        <CardContent className="space-y-6">
          <div>
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div><span className="text-gray-600">Category:</span> {typeData.category?.replace('_', ' ')}</div>
              <div><span className="text-gray-600">Subcategory:</span> {typeData.subcategory}</div>
              <div><span className="text-gray-600">Confidential:</span> {typeData.isConfidential ? 'Yes' : 'No'}</div>
              <div><span className="text-gray-600">Retention:</span> {typeData.retentionPeriod?.replace('_', ' ')}</div>
            </div>
            <div className="mt-2">
              <span className="text-gray-600">Description:</span> {typeData.description}
            </div>
          </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                {metaData.documentDate && (
                  <div><span className="text-gray-600">Document Date:</span> {metaData.documentDate}</div>
                )}
                {metaData.expiryDate && (
                  <div><span className="text-gray-600">Expiry Date:</span> {metaData.expiryDate}</div>
                )}
                {metaData.relatedClient && (
                  <div><span className="text-gray-600">Client:</span> {metaData.relatedClient}</div>
                )}
                {metaData.relatedMatter && (
                  <div><span className="text-gray-600">Matter:</span> {metaData.relatedMatter}</div>
                )}
              </div>
            </div>
          )}
        ...
        <CardContent className="space-y-4">
          <div className="flex items-start space-x-3">
            <Checkbox
              ...
              onCheckedChange={(checked) => handleChange("confirmAccuracy", checked as boolean)}
            />
            <div>
          ...
          <div className="flex items-start space-x-3">
            <Checkbox
              ...
              onCheckedChange={(checked) => handleChange("confirmCompliance", checked as boolean)}
            />
            <div>
              <Label>I confirm compliance with document retention policies</Label>
              <p className="text-sm text-gray-600 mt-1">
                Documents comply with regulatory and company retention requirements
              </p>
            </div>
          </div>
          ...
          <div className="flex items-start space-x-3">
            <Checkbox
              ...
              onCheckedChange={(checked) => handleChange("notifyClient", checked as boolean)}
            />
            <div>
