...
interface BusinessProfile {
  businessType: "sole_proprietorship" | "partnership" | "company" | "ngo" | "other";
  hasEmployees: boolean;
...
const complianceItems: ComplianceItem[] = [
  {
    ...
    description: "File annual income tax returns with GRA by the statutory deadline",
    category: "tax",
    priority: "critical",
    frequency: "annually",
    applicableIf: {},
...
export function SmartComplianceChecklist({
  ...
  const applicableItems = useMemo(() => {
    return complianceItems.filter(item => {
      ...
      // Check business type
      if (applicableIf.businessType && !applicableIf.businessType.includes(profile.businessType)) {
        return false;
      }

      // Check employee requirement
      if (applicableIf.hasEmployees !== undefined && applicableIf.hasEmployees !== profile.hasEmployees) {
        return false;
      }

      // Check annual revenue
      if (applicableIf.annualRevenue && !applicableIf.annualRevenue.includes(profile.annualRevenue)) {
        return false;
      }

      // Check environmental impact
      if (applicableIf.hasEnvironmentalImpact !== undefined && applicableIf.hasEnvironmentalImpact !== profile.hasEnvironmentalImpact) {
        return false;
      }
  ...
  const stats = useMemo(() => {
    ...
    const totalTasks = applicableItems.reduce((sum, item) => sum + item.tasks.length, 0);
    ...
    const criticalItems = applicableItems.filter(item => item.priority === "critical");
    ...
    return {
      ...
      completedTasksCount,
      completedItemsCount,
      taskCompletionPercentage: totalTasks > 0 ? Math.round((completedTasksCount / totalTasks) * 100) : 0,
      itemCompletionPercentage: totalItems > 0 ? Math.round((completedItemsCount / totalItems) * 100) : 0,
  ...
  const handleTaskCompletion = (itemId: string, taskId: string, completed: boolean) => {
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      ...
      case "environmental": return <Target className="h-4 w-4" />;
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold text-gray-900">Smart Compliance Checklist</h1>
          ...
          <CardContent className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div>
              ...
              <Select
                ...
                onValueChange={(value: any) => handleProfileUpdate("businessType", value)}
              >
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value: any) => handleProfileUpdate("annualRevenue", value)}
              >
                <SelectTrigger className="mt-2">
                  <SelectValue />
                </SelectTrigger>
            ...
            <div className="flex items-center space-x-2">
              <Checkbox
                ...
                onCheckedChange={(checked) => handleProfileUpdate("hasEmployees", checked)}
              />
            ...
            {profile.hasEmployees && (
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => handleProfileUpdate("employeeCount", parseInt(e.target.value) || 0)}
                  className="mt-2"
                />
              </div>
            )}
            ...
            <div className="flex items-center space-x-2">
              <Checkbox
                ...
                onCheckedChange={(checked) => handleProfileUpdate("hasEnvironmentalImpact", checked)}
              />
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{stats.taskCompletionPercentage}%</p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{stats.completedItemsCount}/{stats.totalItems}</p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{stats.completedCriticalItemsCount}/{stats.criticalItemsCount}</p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{stats.completedTasksCount}/{stats.totalTasks}</p>
              </div>
      ...
      <div className="space-y-4">
        {applicableItems
          .filter(item => showCompleted || !item.tasks.every(task => task.completed))
          .map(item => {
            const isCompleted = item.tasks.every(task => task.completed);
            const completedTasksCount = item.tasks.filter(task => task.completed).length;

            return (
              <Card key={item.id} className={`${isCompleted ? "border-green-200 bg-green-50" : ""}`}>
                <CardHeader>
                        ...
                        <div key={task.id} className="flex items-start gap-3 p-3 border rounded-lg">
                          <Checkbox
                            ...
                            onCheckedChange={(checked) =>
                              handleTaskCompletion(item.id, task.id, checked as boolean)
                            }
                            disabled={readOnly}
                          />
                          <div className="flex-1">
                            <Label className={`text-sm ${task.completed ? "line-through text-gray-500" : ""}`}>
                              {task.description}
                            </Label>
                            {!readOnly && (
                              <Textarea
                                ...
                                value={task.notes}
                                onChange={(e) => handleTaskNotes(item.id, task.id, e.target.value)}
                                className="mt-2 text-xs"
                                rows={2}
                              />
                            )}
                          </div>
                          {task.completed && <CheckCircle className="h-4 w-4 text-green-500 mt-1" />}
                        </div>
                      ))}
                    </div>
                      ...
                      <div className="border-t pt-3">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {item.resources.map((resource, index) => (
                            ...
                            >
                              {resource.type === "form" && <FileText className="h-3 w-3 mr-1" />}
                              {resource.type === "guide" && <HelpCircle className="h-3 w-3 mr-1" />}
                              {resource.title}
                            </Button>
                          ))}
                        </div>
      ...
      {applicableItems.length === 0 && (
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <h3 className="text-lg font-medium text-gray-900">No applicable compliance items</h3>
            <p className="text-gray-600 mt-2">
              Based on your business profile, no additional compliance items are currently required.
            </p>
          </CardContent>
        </Card>
      )}
