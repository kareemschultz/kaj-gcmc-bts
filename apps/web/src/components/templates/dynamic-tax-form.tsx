...
interface FormField {
  id: string;
  label: string;
  type: "text" | "number" | "select" | "checkbox" | "textarea" | "date" | "currency";
  required?: boolean;
    ...
    },
    {
      ...
      fields: [
        ...
        },
        {
          ...
          type: "currency",
          calculation: {
            formula: "employment_income + business_income + rental_income + investment_income + other_income",
            dependencies: ["employment_income", "business_income", "rental_income", "investment_income", "other_income"]
          }
        }
      ]
    },
    {
      ...
      fields: [
        ...
        },
        {
          ...
          type: "currency",
          calculation: {
            formula: "personal_allowance + spouse_allowance + medical_expenses + education_expenses + charitable_donations + mortgage_interest",
            dependencies: ["personal_allowance", "spouse_allowance", "medical_expenses", "education_expenses", "charitable_donations", "mortgage_interest"]
          }
        }
      ]
    },
    ...
    },
    {
      ...
      fields: [
        ...
        },
        {
          ...
          type: "currency",
          calculation: {
            formula: "cost_of_goods_sold + operating_expenses + depreciation + interest_expense + other_expenses",
            dependencies: ["cost_of_goods_sold", "operating_expenses", "depreciation", "interest_expense", "other_expenses"]
          }
        },
        {
          id: "net_income",
...
export function DynamicTaxForm({
  ...
  const [calculatedFields, setCalculatedFields] = useState<Record<string, number>>({});
  ...
  const calculateFieldValue = (calculation: { formula: string; dependencies: string[] }, data: Record<string, any>): number => {
    ...
    try {
      ...
      dependencies.forEach(dep => {
        const value = parseFloat(data[dep]) || 0;
  ...
  const calculateTaxLiability = (taxableIncome: number): number => {
    ...
    if (taxableIncome <= 1200000) return (taxableIncome - 600000) * 0.20;
    if (taxableIncome <= 1800000) return 120000 + (taxableIncome - 1200000) * 0.25;
    return 270000 + (taxableIncome - 1800000) * 0.30;
  };
  ...
  const shouldShowField = (field: FormField): boolean => {
    ...
    const { field: dependentField, value: expectedValue, condition = "equals" } = field.dependsOn;
    ...
    switch (condition) {
      ...
      case "not_equals":
        return actualValue !== expectedValue;
      case "greater_than":
        return parseFloat(actualValue) > parseFloat(expectedValue as string);
      case "less_than":
        return parseFloat(actualValue) < parseFloat(expectedValue as string);
      default:
        return actualValue === expectedValue;
  ...
  const renderField = (field: FormField) => {
    ...
    return (
      <div key={field.id} className="space-y-2">
        ...
        {field.type === "select" && (
          <Select value={fieldValue} onValueChange={(value) => handleFieldChange(field.id, value)} disabled={readOnly}>
...
export function TaxFormTemplateSelector({
  ...
  const templates = [
    individualTaxFormTemplate,
    companyTaxFormTemplate,
  ];
