...
const AGENCY_CATEGORIES: AgencyCategory[] = [
	{
		...
		description:
			"Tax returns, VAT certificates, income statements, corporation tax forms",
		documentTypes: [
			{
	...
	},
	{
		...
		description:
			"Contribution schedules, employee reports, compliance certificates",
		documentTypes: [
			{
				id: "nis-contribution",
	...
	},
	{
		...
		description:
			"Registration certificates, annual returns, director resolutions",
		documentTypes: [
			{
...
export function SmartDocumentUploader({
	...
	const [selectedAgency, setSelectedAgency] = useState<AgencyType | "ALL">(
		"ALL",
	);
	...
	const analyzeDocument = useCallback(
		async (file: File): Promise<Partial<DocumentFile>> => {
			...
			if (
				...
				fileName.includes("gra") ||
				fileName.includes("income")
			) {
				suggestedCategory = "GRA";
				confidence = 0.8;
				if (fileName.includes("income")) {
					suggestedDocumentType = AGENCY_CATEGORIES[0].documentTypes.find(
						(t) => t.id === "income-tax-return",
					);
				...
				fileName.includes("nis") ||
				fileName.includes("contribution") ||
				...
				fileName.includes("annual") ||
				fileName.includes("dcra")
			) {
			...
			return {
				...
				detectedFields: {
					...
					Agency:
						AGENCY_CATEGORIES.find((a) => a.id === suggestedCategory)?.name ||
						"Other",
				},
				tags: [],
				description: "",
				priority:
					suggestedDocumentType?.urgencyLevel === "critical"
						? "urgent"
						: suggestedDocumentType?.urgencyLevel === "high"
							? "high"
							: "medium",
						...
						setFiles((prev) =>
							prev.map((f) =>
								f.id === documentFile.id
									? {
											...f,
											progress: Math.min(f.progress + Math.random() * 25, 95),
										}
									: f,
							),
						);
	...
	const updateFile = useCallback(
		(id: string, updates: Partial<DocumentFile>) => {
			setFiles((prev) =>
	...
	const filteredFiles = files.filter((file) => {
		const matchesAgency =
			selectedAgency === "ALL" || file.suggestedCategory === selectedAgency;
		const matchesSearch =
			searchTerm === "" ||
	...
	const averageComplianceScore =
		files.length > 0
			? Math.round(
					files.reduce((sum, f) => sum + (f.complianceScore || 0), 0) /
						files.length,
				)
			: 0;
			...
			<Card>
				<CardHeader>
					...
					<p className="text-muted-foreground text-sm">
						Intelligent document categorization with agency-specific processing
						for Guyanese regulatory compliance
					</p>
				</CardHeader>
				<CardContent>
					{/* Drop Zone */}
						...
						>
							<motion.div
								...
								className={cn(
									...
									"border border-primary/20 bg-gradient-to-br from-primary/20 to-purple-500/20",
								)}
							>
							...
							<div>
								<h3 className="mb-2 flex items-center gap-2 font-semibold text-gray-900 text-lg">
								...
								<div className="flex flex-wrap justify-center gap-2 text-gray-500 text-xs">
						...
						<div className="flex items-center justify-between">
							<div>
								...
								<p className="text-muted-foreground text-sm">
									...
									{averageComplianceScore}% avg. compliance
								</p>
							</div>
					...
					<CardContent>
						...
						<div className="mb-6 flex flex-col gap-4 sm:flex-row">
							...
							<Select
								...
								onValueChange={(value: AgencyType | "ALL") =>
						...
						<div
							className={cn(
								"space-y-4",
								viewMode === "grid" &&
									"grid grid-cols-1 gap-4 space-y-0 md:grid-cols-2 xl:grid-cols-3",
							)}
						>
				...
				>
								...
								{file.complianceScore && (
									<>
										...
										<span
											className={cn(
												"font-medium",
												file.complianceScore >= 80
													? "text-green-600"
													: file.complianceScore >= 60
														? "text-yellow-600"
														: "text-red-600",
											)}
										>
					...
					<div
						className={cn(
							...
							viewMode === "grid" && "mt-3 border-t pt-3",
						)}
					>
						...
						{showDetails && viewMode === "grid" && (
							...
							>
								...
								{file.requiredActions && file.requiredActions.length > 0 && (
									<div className="space-y-2">
										<Label className="text-orange-600 text-xs">
											Required Actions
										</Label>
...
function ComplianceRequirementsChecker({
	...
	const getDocumentStatus = (docType: DocumentType) => {
		const hasDoc = documents.some(
			(doc) =>
				doc.suggestedDocumentType?.id === docType.id &&
									...
									return (
										...
										>
											...
											<div className="flex items-center gap-2">
												<Badge
													variant={
														docType.urgencyLevel === "critical"
															? "destructive"
															: docType.urgencyLevel === "high"
																? "default"
																: "secondary"
													}
													className="text-xs"
												...
												<Badge
													variant={
														status === "complete" ? "default" : "destructive"
													}
													className="text-xs"
												>
													{status}
