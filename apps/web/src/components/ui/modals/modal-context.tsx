...
export interface ModalConfig {
  id: string;
  type: 'confirmation' | 'form' | 'gallery' | 'wizard' | 'fullscreen' | 'toast' | 'loading' | 'custom';
  title?: string;
  description?: string;
  content?: ReactNode;
...
export function ModalProvider({ children, maxModals = 5 }: ModalProviderProps) {
  ...
  const closeModal = useCallback((id: string) => {
    setState(prev => {
      ...
      const newActiveModalId = newModals.length > 0 ? newModals[newModals.length - 1].id : null;
  ...
  const contextValue = useMemo((): ModalContextValue => ({
    ...
    getModal,
    isModalOpen,
  }), [state, openModal, closeModal, closeAllModals, updateModal, getModal, isModalOpen]);
...
export function useLoadingModal() {
  ...
  return {
    ...
    updateProgress: useCallback((id: string, progress: number, description?: string) => {
      updateModal(id, {
        props: { progress },
