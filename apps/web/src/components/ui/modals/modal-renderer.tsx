  ...
  return createPortal(
    ...
    >
      <motion.div
        ref={modalRef}
        className={cn(
          'relative w-full mx-4 my-8 bg-background shadow-2xl rounded-lg overflow-hidden',
          sizeClasses[size],
          className
        )}
        variants={variants}
      ...
      >
        ...
        {dismissible && (
          <button
            type="button"
            onClick={onClose}
            className="absolute top-4 right-4 z-10 p-1 rounded-full bg-background/80 hover:bg-background text-muted-foreground hover:text-foreground transition-colors"
...
const ModalContent = memo(function ModalContent({ modal, onClose }: ModalContentProps) {
...
const ConfirmationModalContent = memo(function ConfirmationModalContent({
  ...
  const { confirmText = 'Confirm', cancelText = 'Cancel', variant = 'danger' } = props;
...
const ToastModalContent = memo(function ToastModalContent({
  ...
  return (
    <div className="p-4 flex items-start gap-3">
      <div className={cn('w-2 h-2 rounded-full mt-2', iconClasses[type])} />
