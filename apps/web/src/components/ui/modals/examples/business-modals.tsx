	...
	const handleCreateClient = async () => {
		...
		if (result) {
			notification.success(
				`Client "${result.businessName}" created successfully`,
			);
...
export const TaxCalculationModals = memo(function TaxCalculationModals() {
	...
	const handleQuickTaxEstimate = async () => {
		...
		if (result) {
			...
			notification.success(
				`Estimated tax liability: $${estimatedTax.toLocaleString()}`,
			);
		}
	};
				...
				content: (
					<div className="space-y-4">
						<div className="rounded-lg bg-muted p-4">
							<h4 className="mb-2 font-medium">Tax Return Summary</h4>
						...
						<div className="flex items-center space-x-2">
							...
							<label htmlFor="accuracy" className="text-sm">
								I certify that the information provided is accurate to the best
								of my knowledge
							</label>
						</div>
						<div className="flex items-center space-x-2">
	...
	const calculateTaxEstimate = (data: any) => {
		const income = Number.parseFloat(data.grossIncome) || 0;
		const deductions = Number.parseFloat(data.deductions) || 12950; // Standard deduction
		const taxableIncome = Math.max(0, income - deductions);

		// Simplified tax brackets (2023)
		let tax = 0;
		if (taxableIncome > 539900) tax += (taxableIncome - 539900) * 0.37;
		if (taxableIncome > 215950)
			tax += Math.min(taxableIncome - 215950, 539900 - 215950) * 0.35;
		if (taxableIncome > 182050)
			tax += Math.min(taxableIncome - 182050, 215950 - 182050) * 0.32;
		if (taxableIncome > 95450)
			tax += Math.min(taxableIncome - 95450, 182050 - 95450) * 0.24;
		if (taxableIncome > 44725)
			tax += Math.min(taxableIncome - 44725, 95450 - 44725) * 0.22;
		if (taxableIncome > 11000)
			tax += Math.min(taxableIncome - 11000, 44725 - 11000) * 0.12;
		if (taxableIncome > 0) tax += Math.min(taxableIncome, 11000) * 0.1;
		...
		const result = await formDialog.create({
			...
			fields: [
				...
				},
				{
					...
					type: "textarea",
					placeholder:
						"Please describe your needs or any specific questions...",
					required: false,
				},
			],
		});
