...
export function AgencyComplianceDashboard({ className }: ComplianceDashboardProps) {
	return (
		<div className={cn("space-y-6", className)}>
			{/* Header */}
			<div className="flex items-center justify-between">
				<div>
					<h2 className="font-bold text-3xl tracking-tight">Agency Compliance Dashboard</h2>
			...
			<div className="grid gap-4 md:grid-cols-4">
				<Card className="border-l-4 border-l-brand">
					...
					<CardContent>
						<div className="flex items-center gap-2">
							...
							<span className={cn(
								"text-xs font-medium",
								overallStats.trend === 'up' ? "text-success" : "text-destructive"
							)}>
				...
				<Card className={cn(
					"border-l-4",
					overallStats.totalAlerts > 0 ? "border-l-destructive" : "border-l-success"
				)}>
					<CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
						<CardTitle className="font-medium text-sm">
							Urgent Alerts
						...
						<AlertTriangle className={cn(
							"h-4 w-4",
							overallStats.totalAlerts > 0 ? "text-destructive" : "text-success"
						)} />
					</CardHeader>
					<CardContent>
				...
				<Card className={cn(
					"border-l-4",
					overallStats.totalDeadlines > 5 ? "border-l-warning" : "border-l-info"
				)}>
					<CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
						<CardTitle className="font-medium text-sm">
							Upcoming Deadlines
					...
					<CardContent>
						<div className="font-bold text-2xl">{overallStats.totalDeadlines}</div>
				...
				<Card className="border-l-4 border-l-info">
					...
					<CardContent>
						<div className="font-bold text-2xl">
							{agencyStatuses.reduce((sum, agency) => sum + agency.activeSubmissions, 0)}
						</div>
						<p className="text-muted-foreground text-xs">In progress across agencies</p>
					</CardContent>
				</Card>
			</div>
				...
				<TabsContent value="overview" className="space-y-6">
					...
					<Card>
						<CardHeader>
							...
							<CardDescription>Common compliance tasks across agencies</CardDescription>
						</CardHeader>
						<CardContent>
			...
			<CardContent className="space-y-4">
				...
				<div>
					<div className="mb-2 flex items-center justify-between">
						...
						<div className="flex items-center gap-1">
							<span className="font-bold text-lg">{agency.complianceScore}%</span>
							{agency.trend === 'up' ? (
					...
					<div className="mt-1 flex justify-between text-xs text-muted-foreground">
						<span>
							{agency.trend === 'up' ? '+' : agency.trend === 'down' ? '-' : '±'}
				...
				<div className="grid grid-cols-3 gap-4 text-center">
					<div>
						<div className={cn(
							"font-bold text-lg",
							agency.urgentAlerts > 0 ? "text-destructive" : "text-muted-foreground"
						)}>
							{agency.urgentAlerts}
						</div>
					...
					<div>
						<div className={cn(
							"font-bold text-lg",
							agency.upcomingDeadlines > 5 ? "text-warning" : "text-muted-foreground"
						)}>
...
function StatusBadge({ status }: { status: AgencyStatus['status'] }) {
	const variants = {
		excellent: { variant: 'success' as const, icon: CheckCircle2, label: 'Excellent' },
	...
	return (
		<Card>
			<CardHeader>
				...
				<CardDescription>Critical filing dates across all agencies</CardDescription>
			</CardHeader>
			<CardContent>
				<div className="space-y-4">
					{/* Add deadline calendar implementation */}
					<p className="text-muted-foreground">
						Deadline calendar implementation would go here with dates, agencies, and client counts.
					</p>
				</div>
			</CardContent>
		</Card>
	...
	return (
		<Card>
			<CardHeader>
				...
				<CardDescription>Urgent items requiring immediate attention</CardDescription>
			</CardHeader>
			<CardContent>
				<div className="space-y-4">
	...
	return (
		<Card>
			<CardHeader>
				...
				<CardDescription>Dependencies and workflow visualization</CardDescription>
			</CardHeader>
			<CardContent>
				<div className="space-y-4">
