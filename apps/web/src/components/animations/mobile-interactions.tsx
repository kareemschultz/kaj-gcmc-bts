...
export function PullToRefresh({
	...
	return (
		<div className="relative overflow-hidden">
			...
			{shouldAnimate && (
				<motion.div
					className="-mt-16 absolute top-0 right-0 left-0 z-10 flex h-16 items-center justify-center bg-white"
					style={{
			...
			{/* Content */}
			<motion.div className="relative" {...dragProps}>
				{children}
			</motion.div>
		</div>
	);
	...
	const getFeedbackProps = () => {
		...
		switch (feedbackType) {
			...
			case "opacity":
				return {
					animate: isTouching
						? { opacity: intensityValues[intensity].opacity }
						: { opacity: 1 },
					transition: { duration: 0.1, ease: "easeOut" },
				};
	...
	return (
		<div className="h-full w-full touch-none overflow-hidden">
			<motion.div
				...
				className="h-full w-full origin-center cursor-grab active:cursor-grabbing"
			>
				{children}
			</motion.div>
		</div>
