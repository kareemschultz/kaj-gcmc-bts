...
>(
	(
		...
		const labelMotionProps = shouldAnimate
			? {
					animate:
						labelVariants[floatingLabel ? "floating" : "static"][
							getLabelState()
						],
					transition: { duration: 0.2, ease: "easeOut" },
				}
		...
		return (
			...
			>
				...
				<div className="relative">
						...
						<Input
							...
							className={cn(
								...
								animationType === "minimal" &&
									"rounded-none border-0 border-b-2 bg-transparent focus-visible:ring-0",
					...
					{floatingLabel && label && (
						<motion.div
							className="pointer-events-none absolute top-3 left-3 origin-left"
							{...labelMotionProps}
						>
							<Label
								className={cn(
					...
					{/* Status Icons */}
					<div className="-translate-y-1/2 absolute top-1/2 right-3 flex items-center space-x-2">
