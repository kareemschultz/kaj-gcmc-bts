...
export class NavigationComponent extends BasePage {
	...
	constructor(page: Page) {
		...
		this.clientsLink = page.locator(
			'[data-testid="nav-clients"], a[href*="/clients"]:has-text("Clients")',
		);
		this.filingsLink = page.locator(
			'[data-testid="nav-filings"], a[href*="/filings"]:has-text("Filings")',
		);
		this.reportsLink = page.locator(
			'[data-testid="nav-reports"], a[href*="/reports"]:has-text("Reports")',
		);
		this.tasksLink = page.locator(
			'[data-testid="nav-tasks"], a[href*="/tasks"]:has-text("Tasks")',
		);
		this.complianceLink = page.locator(
			'[data-testid="nav-compliance"], a[href*="/compliance"]:has-text("Compliance")',
		);
		this.notificationsLink = page.locator(
			'[data-testid="nav-notifications"], a[href*="/notifications"]:has-text("Notifications")',
		);
		this.settingsLink = page.locator(
			'[data-testid="nav-settings"], a[href*="/settings"]:has-text("Settings")',
		);

		// Initialize user menu items
		this.profileLink = page.locator(
			'[data-testid="profile-link"], a:has-text("Profile")',
		);
		this.settingsMenuLink = page.locator(
			'[data-testid="settings-link"], a:has-text("Settings")',
		);
		this.logoutLink = page.locator(
			'[data-testid="logout-link"], a:has-text("Logout"), button:has-text("Logout")',
		);
		...
		this.breadcrumbHome = page.locator(
			'.breadcrumb a:has-text("Home"), [data-testid="breadcrumb-home"]',
		);
	...
	async navigateViaBreadcrumb(itemText: string): Promise<void> {
		const breadcrumbItem = this.page.locator(
			`.breadcrumb a:has-text("${itemText}"), [data-testid="breadcrumb-${itemText.toLowerCase()}"]`,
		);
	...
	async verifyActiveNavigation(expectedPage: string): Promise<void> {
		const activeLink = this.page.locator(
			`[data-testid="nav-${expectedPage}"].active, .nav-item.active:has-text("${expectedPage}")`,
		);
	...
	async getCurrentNavigationState(): Promise<{
		...
		if (badgeVisible) {
			const badgeText = await this.getText(this.notificationBadge);
			notificationCount = Number.parseInt(badgeText) || 0;
		}
