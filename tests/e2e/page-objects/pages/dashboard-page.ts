...
export class DashboardPage extends BasePage {
	...
	constructor(page: Page) {
		...
		this.pageHeader = page.locator(
			'[data-testid="page-header"], .page-header, header',
		);
		this.userDropdown = page.locator(
			'[data-testid="user-dropdown"], .user-dropdown, .profile-menu',
		);
		this.logoutButton = page.locator(
			'[data-testid="logout"], button:has-text("Logout"), button:has-text("Sign Out")',
		);
		this.profileButton = page.locator(
			'[data-testid="profile"], button:has-text("Profile")',
		);

		// Initialize navigation locators
		this.navClients = page.locator(
			'a[href*="/clients"], nav >> text="Clients"',
		);
		this.navFilings = page.locator(
			'a[href*="/filings"], nav >> text="Filings"',
		);
		this.navReports = page.locator(
			'a[href*="/reports"], nav >> text="Reports"',
		);
		this.navTasks = page.locator('a[href*="/tasks"], nav >> text="Tasks"');
		this.navCompliance = page.locator(
			'a[href*="/compliance"], nav >> text="Compliance"',
		);
		this.navNotifications = page.locator(
			'a[href*="/notifications"], nav >> text="Notifications"',
		);
		this.mobileMenuToggle = page.locator(
			'[data-testid="mobile-menu"], .mobile-menu-toggle, .hamburger',
		);

		// Initialize dashboard content locators
		this.welcomeMessage = page.locator(
			'[data-testid="welcome"], .welcome-message, h1:has-text("Welcome")',
		);
		this.statsCards = page.locator(
			'[data-testid="stats-cards"], .stats-grid, .dashboard-stats',
		);
		this.recentActivity = page.locator(
			'[data-testid="recent-activity"], .recent-activity, .activity-feed',
		);
		this.quickActions = page.locator(
			'[data-testid="quick-actions"], .quick-actions, .action-buttons',
		);
		this.complianceOverview = page.locator(
			'[data-testid="compliance-overview"], .compliance-overview',
		);
		this.taxFilingWidgets = page.locator(
			'[data-testid="tax-filing-widgets"], .tax-filing-widgets',
		);

		// Initialize stats card locators
		this.totalClientsCard = page.locator(
			'[data-testid="total-clients"], .stat-card:has-text("Clients")',
		);
		this.activeFilingsCard = page.locator(
			'[data-testid="active-filings"], .stat-card:has-text("Filings")',
		);
		this.pendingTasksCard = page.locator(
			'[data-testid="pending-tasks"], .stat-card:has-text("Tasks")',
		);
		this.complianceStatusCard = page.locator(
			'[data-testid="compliance-status"], .stat-card:has-text("Compliance")',
		);

		// Initialize table locators
		this.dataTable = page.locator(
			'[data-testid="data-table"], table, .data-table',
		);
		...
		this.filterDropdown = page.locator(
			'[data-testid="filter"], select[name="filter"], .filter-dropdown',
		);
		this.actionButtons = page.locator(
			'[data-testid="action-buttons"], .action-buttons',
		);
		this.modalDialog = page.locator(
	...
	async verifySearchFunctionality(): Promise<void> {
		...
		if (searchExists) {
			...
			const placeholder = await this.getAttribute(
				this.searchBox,
				"placeholder",
			);
