...
export class ClientsPage extends BasePage {
	...
	constructor(page: Page) {
		...
		this.pageTitle = page.locator(
			'h1:has-text("Clients"), [data-testid="page-title"]',
		);
		this.addClientButton = page.locator(
			'button:has-text("Add Client"), [data-testid="add-client"], .btn-add-client',
		);
		this.clientsList = page.locator(
			'[data-testid="clients-list"], .clients-list, .client-container',
		);
		this.searchBox = page.locator(
			'input[placeholder*="Search"], input[type="search"], [data-testid="search"]',
		);
		this.filterDropdown = page.locator(
			'select[name="filter"], [data-testid="filter"], .filter-dropdown',
		);

		// Initialize client table elements
		this.clientsTable = page.locator(
			'table, [data-testid="clients-table"], .clients-table',
		);
		...
		this.clientRow = (clientName: string) =>
			page.locator(
				`tr:has-text("${clientName}"), [data-testid="client-${clientName}"]`,
			);
		this.editButton = (row: number) =>
			page.locator(
				`tbody tr:nth-child(${row + 1}) button:has-text("Edit"), tbody tr:nth-child(${row + 1}) [data-testid="edit"]`,
			);
		this.deleteButton = (row: number) =>
			page.locator(
				`tbody tr:nth-child(${row + 1}) button:has-text("Delete"), tbody tr:nth-child(${row + 1}) [data-testid="delete"]`,
			);
		this.viewButton = (row: number) =>
			page.locator(
				`tbody tr:nth-child(${row + 1}) button:has-text("View"), tbody tr:nth-child(${row + 1}) [data-testid="view"]`,
			);

		// Initialize client form elements
		this.clientForm = page.locator(
			'form, [data-testid="client-form"], .client-form',
		);
		this.nameField = page.locator(
			'input[name="name"], input[name="fullName"], [data-testid="name"]',
		);
		...
		this.addressField = page.locator(
			'textarea[name="address"], input[name="address"], [data-testid="address"]',
		);
		this.companyField = page.locator(
			'input[name="company"], input[name="companyName"], [data-testid="company"]',
		);
		this.taxIdField = page.locator(
			'input[name="taxId"], input[name="tin"], [data-testid="tax-id"]',
		);
		this.statusSelect = page.locator(
			'select[name="status"], [data-testid="status"]',
		);
		this.notesField = page.locator(
			'textarea[name="notes"], [data-testid="notes"]',
		);
		this.saveButton = page.locator(
			'button[type="submit"], button:has-text("Save"), [data-testid="save"]',
		);
		...
		this.confirmDeleteButton = page.locator(
			'button:has-text("Delete"), button:has-text("Confirm"), [data-testid="confirm-delete"]',
		);
		this.cancelDeleteButton = page.locator(
			'button:has-text("Cancel"), [data-testid="cancel-delete"]',
		);
		this.successMessage = page.locator(
			'.alert-success, .success, [data-testid="success"]',
		);
		this.errorMessage = page.locator(
			'.alert-error, .error, [data-testid="error"]',
		);
		...
		this.previousPageButton = page.locator(
			'button:has-text("Previous"), .pagination-prev',
		);
	...
	async sortClientsByColumn(columnName: string): Promise<void> {
		const sortButton = this.page.locator(
			`[data-testid="sort-${columnName}"], th:has-text("${columnName}") button`,
		);
