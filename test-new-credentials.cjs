...
async function testNewCredentials() {
    ...
    try {
        ...
        const demoCredentialsExist = await page.$('.rounded-lg.border.border-emerald-500\\/30.bg-emerald-900\\/30');
        ...
        try {
            ...
            if (page.url().includes('/dashboard')) {
                ...
                const errorElements = await page.$$('.text-red-400, .text-red-500, .text-red-600');
                ...
                console.log(`❌ Login did not redirect to dashboard. Current URL: ${page.url()}`);
                return false;
            }
            ...
            const errorMessages = await page.$$('.text-red-400, .text-red-500, .text-red-600');
...
async function main() {
    ...
    // Summary of changes
    console.log('\n📋 CHANGES MADE:');
    console.log('  • Updated admin password from Admin123!@# to AdminPassword123');
