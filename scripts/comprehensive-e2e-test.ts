...
class E2ETestRunner {
  ...
  async testAuthentication(): Promise<TestSuite> {
    ...
    tests.push(await this.runTest('Load login page', async () => {
      await this.page!.goto(CONFIG.baseUrl);
      await this.page!.waitForSelector('form', { timeout: 10000 });

      // Verify new UI elements
      const hasLogo = await this.page!.locator('[data-testid="logo"], .logo, h1').count() > 0;
      if (!hasLogo) throw new Error('Logo not found on login page');
    }));

    tests.push(await this.runTest('Verify login form UI elements', async () => {
    ...
    tests.push(await this.runTest('Login with admin credentials', async () => {
      await this.page!.fill('input[type="email"]', CONFIG.adminCredentials.email);
      await this.page!.fill('input[type="password"]', CONFIG.adminCredentials.password);
    ...
    tests.push(await this.runTest('Verify session persistence', async () => {
      // Reload page
  ...
  async testDashboard(): Promise<TestSuite> {
    ...
    tests.push(await this.runTest('Load dashboard', async () => {
      await this.page!.goto(`${CONFIG.baseUrl}/dashboard`);
      await this.page!.waitForSelector('main', { timeout: 10000 });
    }));

    tests.push(await this.runTest('Verify stats cards are visible', async () => {
      // Wait for stats cards (at least 3)
      const cards = this.page!.locator('[data-testid="stat-card"], .card, [class*="Card"]');
      const count = await cards.count();
      if (count < 3) throw new Error(`Expected at least 3 stat cards, found ${count}`);
    }));

    tests.push(await this.runTest('Verify navigation menu', async () => {
    ...
    tests.push(await this.runTest('Verify user menu/avatar', async () => {
      const userMenu = this.page!.locator('[data-testid="user-menu"], [aria-label*="user"], button:has-text("admin")').first();
  ...
  async testClientManagement(): Promise<TestSuite> {
    ...
    tests.push(await this.runTest('Navigate to clients page', async () => {
      const clientsLink = this.page!.locator('a:has-text("Clients")').first();
      await clientsLink.click();
      await this.page!.waitForURL('**/clients', { timeout: 10000 });
    }));

    tests.push(await this.runTest('Verify clients list loads', async () => {
      // Wait for either clients table/grid or empty state
      await this.page!.waitForSelector('[data-testid="clients-list"], table, .grid, [data-testid="empty-state"]', { timeout: 10000 });
    }));

    tests.push(await this.runTest('Check search functionality', async () => {
    ...
    tests.push(await this.runTest('Verify "Create Client" button exists', async () => {
      const createButton = this.page!.locator('button:has-text("Create"), button:has-text("Add"), button:has-text("New")').first();
  ...
  async testDocumentManagement(): Promise<TestSuite> {
    ...
    tests.push(await this.runTest('Navigate to documents page', async () => {
      const docsLink = this.page!.locator('a:has-text("Documents")').first();
      await docsLink.click();
      await this.page!.waitForURL('**/documents', { timeout: 10000 });
    }));

    tests.push(await this.runTest('Verify documents list or empty state', async () => {
      await this.page!.waitForSelector('[data-testid="documents-list"], table, .grid, [data-testid="empty-state"]', { timeout: 10000 });
    }));

    tests.push(await this.runTest('Check filter/sort controls', async () => {
      // Look for filter or sort controls
      const controls = this.page!.locator('select, [role="combobox"], button:has-text("Filter"), button:has-text("Sort")');
      // At least one control should exist
      const count = await controls.count();
      if (count === 0) {
        console.log('     ⚠️  No filter/sort controls found (might be by design)');
      }
    }));
  ...
  async testUIDesignSystem(): Promise<TestSuite> {
    ...
    tests.push(await this.runTest('Verify modern color scheme (blue-gray)', async () => {
      await this.page!.goto(`${CONFIG.baseUrl}/dashboard`);
    ...
    tests.push(await this.runTest('Verify responsive design (mobile)', async () => {
      // Set mobile viewport
      ...
      const mobileMenu = this.page!.locator('[data-testid="mobile-menu"], button[aria-label*="menu"]').first();
      ...
      if (!hasMobileMenu) {
        console.log('     ⚠️  Mobile menu not found (check responsive design)');
      }
    }));

    tests.push(await this.runTest('Verify gradient cards exist', async () => {
      await this.page!.goto(`${CONFIG.baseUrl}/dashboard`);

      // Look for modern card elements
      const cards = this.page!.locator('[class*="gradient"], [class*="Card"], .card');
      const count = await cards.count();
    ...
    tests.push(await this.runTest('Verify animations and transitions', async () => {
  ...
  async testAccessibility(): Promise<TestSuite> {
    ...
    tests.push(await this.runTest('Verify keyboard navigation', async () => {
      await this.page!.goto(`${CONFIG.baseUrl}/dashboard`);
    ...
    tests.push(await this.runTest('Check for alt text on images', async () => {
    ...
    tests.push(await this.runTest('Verify ARIA labels on interactive elements', async () => {
      const buttons = this.page!.locator('button[aria-label], a[aria-label]');
  ...
  async runAllTests(): Promise<ComprehensiveReport> {
    ...
    try {
      ...
      const passedTests = this.suites.reduce((sum, suite) =>
        sum + suite.tests.filter(t => t.status === 'PASS').length, 0
      );
      const failedTests = this.suites.reduce((sum, suite) =>
        sum + suite.tests.filter(t => t.status === 'FAIL').length, 0
      );
      const skippedTests = this.suites.reduce((sum, suite) =>
        sum + suite.tests.filter(t => t.status === 'SKIP').length, 0
      );
...
async function main() {
  ...
  try {
    ...
    for (const suite of report.suites) {
      ...
      console.log(`  ${suite.name}: ${passed}/${suite.tests.length} (${suite.passRate.toFixed(1)}%)`);
    }
