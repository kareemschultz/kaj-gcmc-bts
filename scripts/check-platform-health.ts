...
class HealthChecker {
  ...
  async checkPrismaClient(): Promise<HealthCheck> {
    try {
      const { stdout } = await execAsync('ls node_modules/.prisma/client 2>/dev/null || echo "not found"');
  ...
  async checkBuildArtifacts(): Promise<HealthCheck> {
    try {
      const { stdout } = await execAsync('find packages -name "dist" -type d 2>/dev/null | wc -l');
      const count = parseInt(stdout.trim());
  ...
  async checkDocumentation(): Promise<HealthCheck> {
    try {
      const { stdout } = await execAsync('find . -name "*.md" -not -path "./node_modules/*" | wc -l');
      const count = parseInt(stdout.trim());
  ...
  async checkUIComponents(): Promise<HealthCheck> {
    try {
      const { stdout } = await execAsync('find apps/web/src/components -name "*.tsx" 2>/dev/null | wc -l');
      const count = parseInt(stdout.trim());
  ...
  async checkAPIRouters(): Promise<HealthCheck> {
    try {
      const { stdout } = await execAsync('find packages/api/src/routers -name "*.ts" 2>/dev/null | wc -l');
      const count = parseInt(stdout.trim());
  ...
  async checkTests(): Promise<HealthCheck> {
    try {
      const { stdout } = await execAsync('find . -name "*.test.ts" -o -name "*.spec.ts" 2>/dev/null | grep -v node_modules | wc -l');
      const count = parseInt(stdout.trim());
  ...
  async checkGitStatus(): Promise<HealthCheck> {
    try {
      const { stdout } = await execAsync('git status --porcelain 2>/dev/null | wc -l');
  ...
  printReport(report: PlatformHealth) {
    ...
    console.log('\n' + '─'.repeat(64));
    console.log(`Summary: ${report.summary.healthy}/${report.summary.total} checks passed`);
    console.log(`Timestamp: ${report.timestamp}\n`);

    // Print recommendations
    const failedChecks = report.checks.filter(c => c.status === 'DOWN' || c.status === 'DEGRADED');
