...
async function verifyApp() {
  ...
  try {
    ...
    if (pageContent.includes('GCMC-KAJ')) {
      ...
      const hasTitle = await page.$eval('h1', el => el.textContent).catch(() => null);
      console.log(`   Title: ${hasTitle}`);

      const hasTagline = await page.$eval('p', el => el.textContent).catch(() => null);
    ...
    if (loginButton) {
      ...
      const errorElement = await page.$('.error, .text-destructive, [role="alert"]');
      if (errorElement) {
    ...
    const hasEmeraldAccent = await page.$('.text-emerald-400, .from-emerald-600');
