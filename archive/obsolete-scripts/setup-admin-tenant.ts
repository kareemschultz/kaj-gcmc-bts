...
async function setupAdminTenant() {
	try {
		...
		if (!adminUser) {
			throw new Error(
				`Admin user with email ${adminEmail} not found. Please create user first.`,
			);
		}
		...
		if (!defaultTenant) {
			...
			console.log(
				`✅ Created tenant: ${defaultTenant.name} (${defaultTenant.code})`,
			);
		}
		...
		console.log(
			`✅ Successfully assigned admin user to tenant "${defaultTenant.code}" with Admin role`,
		);
		...
		if (verification) {
			...
			console.log(
				`   Permissions: ${verification.role.permissions.length} permissions granted`,
			);
		}
